...
export const METRICS = {
  ...
  dbTables: 105,       // Drizzle table declarations
  // ...
} as const;
```
...
That hard failure is the whole point. The numbers you read here are not a snapshot someone updated when they remembered. They are a measurement of the code as it exists right now: 32 packages, 66 UI components, 13 first-party MCP servers, 105 database tables, 60 access-control enforcement tests, 5 starter templates. Each one is checked on the commit that publishes it.
