...
# =============================================================================
# REQUIRED - STORAGE (Cloudflare R2 — canonical, S3-compatible)
# =============================================================================
...
| `R2_ACCOUNT_ID` | [Cloudflare Dashboard](https://dash.cloudflare.com)  -  the account ID shown on the R2 overview page |
| `R2_ACCESS_KEY_ID` / `R2_SECRET_ACCESS_KEY` | Cloudflare Dashboard  -  R2 → Manage R2 API Tokens → Create API Token (S3-compatible credentials) |
| `R2_BUCKET` | The name of the R2 bucket you created |
| `R2_PUBLIC_BASE_URL` | A bound custom domain (e.g. `https://media.your-domain.com`) or the dev URL `https://<account-id>.r2.cloudflarestorage.com/<bucket>` |
...
RevealUI ships with 14 first-party MCP (Model Context Protocol) servers (including Stripe, Supabase, Neon, Vercel, Playwright, Code Validator, and Next.js DevTools). See the full list in [`packages/mcp/src/servers/`](https://github.com/RevealUIStudio/revealui/tree/main/packages/mcp/src/servers). Let your AI agents interact directly with your business infrastructure.
