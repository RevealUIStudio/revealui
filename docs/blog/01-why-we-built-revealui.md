...
**It's pre-launch.** This is a v0. The core is stable (extensive test coverage, full TypeScript strict mode, comprehensive security hardening — run `pnpm test` for the current count), but there are zero paying customers yet, and Stripe runs in test mode in production until a billing-readiness audit closes. There's no plugin marketplace yet. The template library is small. The community is just getting started. I keep an honest, file-by-file account of what does and doesn't work at [What Works Today](../WHAT_WORKS_TODAY.md).
