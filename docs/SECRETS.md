...
| `revealui/prod/audit-hmac-secret` | credential | yes | vercel:api, fly:worker | RETIRED (GAP-355 Stage 3) - audit signing moved to per-row Ed25519 (REVEALUI_AUDIT_SIGNING_KEY); this HMAC secret has no code consumer. Physical path removal + the signing-key add land together in Stage 3 PR-3. |
...
| `revealui/staging/audit-hmac-secret` | credential | yes | vercel:api-staging | RETIRED (GAP-355 Stage 3) - superseded by per-row Ed25519 signing (REVEALUI_AUDIT_SIGNING_KEY); no code consumer. Removed with the signing-key add in Stage 3 PR-3. |
