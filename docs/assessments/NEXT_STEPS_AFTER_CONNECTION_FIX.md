...
### 2. Set Up Both Databases (15 minutes)

**Issue:** `agent_memories` table doesn't exist in Supabase
...
### Step 2: Set Up Supabase Schema (Fresh Setup)

The setup script (`packages/test/scripts/setup-vector-database.ts`) will:
1. Check if `pgvector` extension exists
2. Check if `agent_memories` table exists
3. Run fresh schema setup from `packages/db/migrations/supabase-vector-setup.sql`
4. Verify setup
...
- ✅ Supabase schema set up (pgvector + agent_memories table)
- ✅ All 115 tests passing
- ✅ Vector memory operations working
- ✅ Episodic memory operations working
- ✅ Dual database architecture fully verified
