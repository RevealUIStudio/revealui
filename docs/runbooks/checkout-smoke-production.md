...
- [ ] **Margin governor must not waitlist the smoke user.** Keep `MARGIN_GOVERNOR_SHADOW=true` (or `MARGIN_GOVERNOR_ENABLED` unset) for this smoke. A `202 WAITLISTED` on free signup is governor enforce, not a license-cap bug. Paid checkout uses `paid_signup` (bypass). Flag matrix: [`margin-governor-enforce.md`](./margin-governor-enforce.md).
