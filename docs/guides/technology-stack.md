...
| Choice | Role | Why |
|---|---|---|
| **NeonDB** (Postgres) | Primary database — 98 tables across the five primitives | Serverless Postgres; pgvector supported; any standard Postgres works as a substitute (the agnosticism principle is a contract, not a coupling). |
