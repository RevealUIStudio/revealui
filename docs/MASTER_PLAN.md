...
- [ ] CMS content CRUD E2E — IN PROGRESS (Session 30): fixes code-complete (`40429f53`), blocked on Vercel build rate limit
  ...
  - **Server-side field hooks don't run in `revealui.create()`** — `packages/core/src/api/rest.ts` POST calls `revealui.create()` but field-level `beforeValidate` hooks (slug auto-generation) do not execute → 500 when slug is absent + required. Client-side workaround: `AdminDashboard.handleSave` detects required slug field and generates slug from title before API call (commit `40429f53`). Underlying fix: wire field hooks into `revealui.create()` (Phase 1 backlog).
  ...
  - **Vercel build rate limit (Hobby plan)** — all 4 Vercel projects failing with `upgradeToPro=build-rate-limit`; last good deploy is `e6580bd8`; code fix `40429f53` is on `main` but not deployed. Re-test after rate limit resets.
