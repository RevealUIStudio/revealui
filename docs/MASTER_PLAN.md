...
#### 2.10 Free Image Generation Research (Google Gemini) ✅ RESEARCH COMPLETE — NOT VIABLE
- [x] Research Google Gemini image generation API — free tier limits, quality, terms (Session 53)
- [x] Research Gemini 2.0 Flash native image generation capabilities (Session 53)
- [x] Evaluate free vs paid tier limits (requests/day, resolution, commercial use rights) (Session 53)
- [x] Determine if generated images can be used commercially by RevealUI customers (Session 53)

> **Research verdict (Session 53):** Free Gemini image generation is **not viable**. `gemini-2.0-flash-exp` (the only model with a free image API) is deprecated and broken as of November 2025. Current models (2.5 Flash Image, 3.x) are paid-only at $0.02–$0.067/image. Free tier has disqualifying data-training terms (Google reviews prompts/outputs). EU users explicitly excluded from free tier. Cheapest viable path: Imagen 4 Fast at $0.02/image on paid API. **Decision: defer image generation to a paid Pro+ feature; BYOK covers customers who want to use their own key with DALL-E or Gemini.**

#### 2.11 Internal vs Productized Boundary
- [x] `scripts/sync-clones.sh` — already removed in a prior session; no machine-specific scripts remain in committed code (Session 53)
- [x] Add branding config to `@revealui/config` — `BrandingConfig` (name, logoUrl, primaryColor, showPoweredBy); reads `REVEALUI_BRAND_*` env vars; wired into `Config` interface + Proxy; Enterprise white-label via `REVEALUI_SHOW_POWERED_BY=false` (Session 53)
- [x] `PoweredByRevealUI` updated — reads `REVEALUI_SHOW_POWERED_BY` env var directly + `REVEALUI_BRAND_NAME` for custom name; no license JWT required to opt out (Session 53)
- [x] CI clean room test — `packages/cli/src/__tests__/template-clean.test.ts`: 4 tests verify template files contain no machine-specific paths, `workspace:*`, `.claude/`, `MASTER_PLAN.md`, `business/`, or founder emails; 27/27 CLI tests passing (Session 53)
- [x] Template `.env.example` updated with commented branding vars section (Session 53)
- [x] Define dogfood instance — `revealui.com` runs on RevealUI CMS; `apps/marketing` for marketing, `apps/cms` for content, `apps/docs` for documentation; separate deployment config from generic product config (Session 55)
...
#### 3.1 Documentation
- [x] Deploy docs site — `revealui-docs` Vercel project; internal docs (MASTER_PLAN, GOVERNANCE, AI-AGENT-RULES, AUTOMATION, CI_ENVIRONMENT, PRICE_COLLECTION, PRODUCT_COLLECTION, SECRETS-MANAGEMENT) filtered from public build; 63 user-facing markdown files served; deployed to `revealui-docs-*.vercel.app` (Session 56)
- [ ] Quick start guide tested by someone other than you — content written: `docs/guides/quick-start.md` (create-revealui CLI, env setup, db:migrate, pnpm dev) (Session 59)
- [ ] API reference complete for core package — skeleton written: `docs/reference/README.md` (all 18 packages + REST API table) (Session 59)
- [ ] At least 3 example projects working

#### 3.2 Open Source Preparation
- [x] License headers verified — 5 OSS packages were missing `"license": "MIT"` in package.json (core, db, router, config, sync); all corrected. Pro packages (ai, mcp, editors, services, harnesses) already had correct commercial license references. `publishConfig.access: "public"` and `files` arrays were already correct on all packages (Session 57)
- [x] CONTRIBUTING.md accurate — fixed stale app names: `dashboard/` (deleted) removed, `landing/` → `marketing/`, `web/` → `mainframe/`, `studio/` added; db table count updated 25+→36 (Session 57)
...
- [x] Session 59 (2026-03-03, WSL): **Phase 3 — cursor migration + docs content + OSS cleanup** — (1) Cursor IDE configs migrated from RevealUI to `RevealUIStudio/editor-configs` repo (`cursor/` dir); `.cursor/`, `.cursorrules`, `.cursorignore`, `packages/harnesses/src/adapters/cursor-adapter.ts` removed from RevealUI; harnesses `CursorAdapter` import/export removed (typecheck clean). (2) Phase 3.1 docs content: `docs/guides/README.md` (guide index), `docs/guides/quick-start.md` (user-facing create-revealui guide), `docs/reference/README.md` (full API reference index for 18 packages + REST API). `copy-docs.sh` updated to exclude internal contributor guides (DOCUMENTATION_QUICK_START, DOCUMENTATION_WORKFLOW_GUIDE, VERCEL_SKILLS). (3) Phase 3.2 OSS cleanup: `packages/dev` + `packages/test` added `"license": "MIT"` (all 13 OSS packages now have correct license field); `LICENSE` copyright updated `2025` → `2025-2026`, `RevealUI Team` → `RevealUI Studio`; `CONTRIBUTING.md` broken link fixed (ENV_FILE_STRATEGY → ENVIRONMENT_VARIABLES_GUIDE). Commits: `c2eb331bb`, `9ea86d545`.
- [x] Session 57 (2026-03-03, WSL): **Phase 3.2 — OSS preparation (partial)** — Smoke-tested `docs.revealui.com`: public docs serve as `text/markdown` ✓, MASTER_PLAN.md not in deployed build ✓. Fixed 5 OSS packages missing `"license": "MIT"` in package.json (core, db, router, config, sync). Updated CONTRIBUTING.md: removed deleted `dashboard/` app, renamed `landing/`→`marketing/`, `web/`→`mainframe/`, added `studio/`, updated db table count (25+→36). Verified CODE_OF_CONDUCT.md (exists), all 4 issue templates + PR template (exist), Pro package.json commercial license references (correct). GitHub repo is still private — making it public requires explicit owner confirmation.
- [x] Session 56 (2026-03-03, WSL): **Phase 3.1 — docs site deployed** — Fixed content security: `vite.config.ts` and `scripts/copy-docs.sh` now filter 8 internal-only top-level docs (MASTER_PLAN, GOVERNANCE, AI-AGENT-RULES, AUTOMATION, CI_ENVIRONMENT, PRICE_COLLECTION, PRODUCT_COLLECTION, SECRETS-MANAGEMENT) from the public build. 63 user-facing markdown files remain. Also exempted `vite.config*.ts` and `vitest.config*.ts` from `no-console-log` code-standards rule (build tooling, not production code). Docs app deployed to Vercel (`revealui-docs` project). Build verified: `dist/docs/MASTER_PLAN.md` excluded ✓, 63 public files ✓. Commit: `1dcaa7861`.
- [x] Session 55 (2026-03-03, WSL): **Phase 2.11 — internal/productized boundary enforcement** — (1) `scripts/validate/boundary.ts`: CI script with 3 checks: OSS packages don't import commercial (ai/mcp/editors/services/harnesses), published source has no internal dev references, package `files` fields exclude internal dirs. Skips private packages. Passes clean. (2) `pnpm validate:boundary` script added to root `package.json`. (3) `Boundary validation` added to gate Phase 1 (warn-only). (4) `apps/cms/deployment/dogfood.env.example`: documents env vars specific to `revealui.com` dogfood deployment vs generic customer deployment (branding, REVEALUI_KEK, ANTHROPIC_API_KEY, enterprise license, signup gating). Phase 2.11 complete. All Phase 2 exit criteria met. Commit: `ae432a4f1`.
- [x] Session 54 (2026-03-03, WSL): **Phase 2.9 BYOK server-side** — (1) `packages/db/src/schema/api-keys.ts`: `userApiKeys` + `tenantProviderConfigs` tables (FK→users, indexes, Drizzle relations). (2) `packages/db/migrations/0013_add_byok_tables.sql`. (3) `packages/db/src/crypto.ts`: AES-256-GCM envelope encryption (`encryptApiKey`, `decryptApiKey`, `redactApiKey`); exported as `@revealui/db/crypto`. (4) `apps/api/src/routes/api-keys.ts`: POST/GET/DELETE/rotate CRUD with OpenAPI docs, auth guard, provider allowlist. (5) `packages/ai/src/llm/client.ts`: `createLLMClientForUser(userId, db)` — queries keys + configs, decrypts key, returns `LLMClient | null`. (6) `/api/api-keys` route mounted in API index. 292/292 tests passing. Commit: `51cd30441`.
- [x] Session 53 (2026-03-03, WSL): **Phase 2.10+2.11 — image gen research + branding config + clean room tests** — (1) Research complete: Gemini free image generation not viable — `gemini-2.0-flash-exp` deprecated Nov 2025, current models paid-only ($0.02–$0.067/image), free tier has data-training clauses disqualifying for SaaS, EU users excluded from free tier. Deferred to paid Pro+ feature. (2) `packages/config/src/modules/branding.ts`: `BrandingConfig` interface (name, logoUrl, primaryColor, showPoweredBy) reading `REVEALUI_BRAND_*` env vars. (3) `packages/config/src/schema.ts`: added `REVEALUI_BRAND_NAME`, `REVEALUI_BRAND_LOGO_URL`, `REVEALUI_BRAND_PRIMARY_COLOR` (hex regex), `REVEALUI_SHOW_POWERED_BY`, `REVEALUI_LICENSE_KEY` to optional schema. (4) `packages/config/src/index.ts`: `Config` interface + Proxy `ownKeys`/`has`/`getOwnPropertyDescriptor` updated with `branding` key; `BrandingConfig` exported. (5) `PoweredByRevealUI`: reads `REVEALUI_SHOW_POWERED_BY` env var directly + `REVEALUI_BRAND_NAME` for custom name. (6) `packages/cli/src/__tests__/template-clean.test.ts`: 4-test clean room suite — 27/27 passing. (7) CLI template `.env.example`: branding section added (commented). Commit: `45e0cfdc`.
