...
- [ ] Verify agent orchestration works end-to-end — set `GROQ_API_KEY` + `LLM_PROVIDER=groq`, `POST /api/agent-tasks` (blocked: GROQ_API_KEY not in terminal env; load via revvault then run `test:integration`; OpenAI not authorised until business has revenue)
