...
import { lexicalEditor, HeadingFeature, BlocksFeature } from '@revealui/core/richtext'
...
const res = await fetch('https://your-cms.com/api/posts?where[status][equals]=published&sort=-publishedAt&limit=10')
...
export const Products = defineCollection({
  ...
  fields: [
    ...
    { name: 'prices', type: 'relationship', relationTo: 'prices', hasMany: true },
...
export const Orders = defineCollection({
  ...
  admin: { useAsTitle: 'id', defaultColumns: ['id', 'status', 'total', 'createdAt'] },
  fields: [
...
export const slugField: Field = {
  ...
  hooks: {
    beforeValidate: [({ data, siblingData }) => {
