...
| **RevForge** | [revforge](https://github.com/RevealUIStudio/revforge) | Self-hosted enterprise deployment kit. Docker Compose stack + per-customer stamp scripts + domain lock + unlimited users. Same kit produces stamped instances (e.g. AlleviaForge for Allevia Technology). Canonical docs at [`/docs/FORGE`](./FORGE). | (per-product LICENSE) | Pre-launch (Docker images not yet on GHCR — stack runs from source today) |
