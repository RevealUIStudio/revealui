...
The boot validator (`apps/server/src/lib/validate-startup.ts`) enforces:
...
Verification dispatch is in [`apps/server/src/middleware/x402.ts:243`](../../apps/server/src/middleware/x402.ts) (`verifyPayment`):
...
For Vercel staging (the canonical apps/server deployment for `test`):
...
### 3 — Boot apps/server and verify the activation banners
...
- Source: [`apps/server/src/middleware/x402.ts`](../../apps/server/src/middleware/x402.ts) (385 LOC; 32 unit tests)
