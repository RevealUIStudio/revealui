...
└───────────────────────┬─────────────────────────────────┘
        ...
        │                               │
┌───────▼───────┐            ┌──────────▼──────────┐
│ types/index.ts │            │ @revealui/schema/core │
...
|--------------|-------------|-------------|
| Base CMS types | `@revealui/core` OR `@revealui/schema/core` | Same types, both work |
| RevealUI extensions | `@revealui/core` | Tenant, permissions, etc. |
| Validation utilities | `@revealui/core` | `validateWithErrors`, etc. |
| Config helpers | `@revealui/core` | `defineCollection`, etc. |

### Examples

```typescript
// ✅ RECOMMENDED: Use @revealui/core for everything
import type { 
...
import type { CollectionConfig, Field } from '@revealui/core';
```
...
```typescript
import { toSlug } from '@revealui/core';
...
```typescript
import { registerCustomFieldType } from '@revealui/core';
...
```typescript
import { registerPluginExtension } from '@revealui/core';
...
```typescript
import { ConfigValidationError, safeValidate } from '@revealui/core';
...
A: Yes. `@revealui/core` re-exports from `@revealui/schema/core`, so both work:
```typescript
import type { CollectionConfig } from '@revealui/core';
import type { CollectionConfig } from '@revealui/schema/core';
```
