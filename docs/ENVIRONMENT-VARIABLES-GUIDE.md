...
The fastest path to a running local dev environment requires **5 variables**. Stripe, object storage, and other services are optional for initial development.
...
| **For uploads** | `R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_BUCKET`, `R2_PUBLIC_BASE_URL` (Cloudflare R2) | When testing media uploads |
| **For AI** | `LLM_PROVIDER`, `OLLAMA_BASE_URL` or `INFERENCE_SNAPS_BASE_URL` | When testing AI agent features |
| **For email** | `GOOGLE_SERVICE_ACCOUNT_EMAIL`, `GOOGLE_WIF_PROVIDER`, `EMAIL_FROM` | When testing password reset or waitlist emails (Gmail API via WIF + Workspace SA) |
...
Cloudflare R2 (S3-compatible) is the object-storage backend (GAP-208). The five `R2_*` vars are resolved together — set all of them, or none. With no backend configured, media uploads fail fast with a clear error (admin still boots; only the upload action errors).

| Variable | Required | Default | Description | Security | Used By |
|----------|----------|---------|-------------|----------|---------|
| `R2_ACCOUNT_ID` | For uploads | None | Cloudflare R2 account ID — the object-storage backend. Set all five `R2_*` vars together. From the Cloudflare dashboard → R2. | HIGH (server-only) | admin, server |
...
| `GOOGLE_WIF_PROVIDER` | No | None | GCP workload identity provider resource name (GAP-211). Paired with `GOOGLE_SERVICE_ACCOUNT_EMAIL`. Vercel injects `VERCEL_OIDC_TOKEN`. | HIGH (server-only) | admin, api |
...
| `REVEALUI_LICENSE_PRIVATE_KEY` | Hosted mint / signer only | None | **Studio mint key** (Ed25519 PEM). Signs customer license JWTs. Never give this to a Fleet customer or put it in `.env.forge` — `REVEALUI_DEPLOYMENT_MODE=forge` refuses to boot if it is present. Canonical vault path `revdev/license-signing-private-key`. | HIGH (server-only) | license-signer, hosted mint |
| `REVEALUI_LICENSE_PUBLIC_KEY` | Fleet + hosted verify | None | Ed25519 PEM **public** key that verifies studio-issued license JWTs. Safe for a Fleet kit. | MEDIUM | admin, api |
| `STRIPE_PROXY` | No | `0` | Set to `1` to enable Stripe proxy logging. Development only. | LOW | admin |
| `STRIPE_TAX_ENABLED` | No | `false` | Set to `true` to gate `automatic_tax` on Checkout sessions once this account's Stripe Tax Settings are active (GAP-437). Not boot-required (fail-open: an account with tax inactive is a legitimate steady state) — but set it explicitly on **both** the Vercel `api` project and the Fly `revealui-worker` app so they can't diverge; the daily `billing-readiness` cron (`apps/server/src/routes/cron/billing-readiness.ts`) and a boot-time worker warning both alert when Stripe Tax is active but this flag isn't. | LOW | api |
...
| Variable | Required | Default | Description | Security | Used By |
|----------|----------|---------|-------------|----------|---------|
| `REVEALUI_LICENSE_KEY` | For Fleet | None | Studio-issued Enterprise/Fleet license JWT. Canonical name in `docker-compose.forge.yml`. | HIGH (server-only) | fleet |
| `REVFORGE_LICENSE_KEY` | Alias | None | Legacy alias for `REVEALUI_LICENSE_KEY`. Compose accepts either. Not a private mint key. | HIGH (server-only) | fleet |
| `REVEALUI_LICENSE_PUBLIC_KEY` | For Fleet | None | Studio public key that verifies the JWT. | MEDIUM | fleet |
| `REVFORGE_LICENSED_DOMAIN` | Recommended | None | Domain this Fleet instance is licensed to serve (e.g., `admin.acme.com`). Requests from other hosts receive HTTP 403. | MEDIUM | fleet |
...
# Object storage — Cloudflare R2 (optional in tests; only needed when testing media uploads)
...
1. Verify the storage backend is configured: all five `R2_*` vars (Cloudflare R2). With none set, uploads fail fast with a clear "no backend configured" error.
2. Confirm the R2 API token has object read/write on the bucket and `R2_PUBLIC_BASE_URL` points at a bound public domain (or the R2 dev URL).
