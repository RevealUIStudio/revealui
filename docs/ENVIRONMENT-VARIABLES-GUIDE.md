...
| `REVEALUI_ALLOW_UNLICENSED_SELF_HOST` | No | `false` | GAP-436: on a self-hosted boot (no `REVEALUI_LICENSE_PRIVATE_KEY`) with no `REVEALUI_LICENSE_KEY` set, set to `true` to boot at Free (OSS) tier instead of refusing to start. Only relaxes a **completely absent** license key — a present-but-invalid/expired key is still rejected regardless of this flag. RevForge-stamped Fleet kits (which always ship a signed license key) are unaffected; this is for the plain OSS self-host path (`docker-compose.yml`, the Railway marketplace template). | LOW | api |
