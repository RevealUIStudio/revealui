...
This document describes the database management scripts available in RevealUI and their usage across different development environments.
...
1. **Always use pnpm scripts** (`pnpm db:*`) for portability
2. **Nix helpers** (`db-*`) are for server control only (start/stop)
3. **Backups are automatic** on reset (unless `--no-backup`)
4. **Connection string** must be in environment variables
5. **Database location** varies by environment (`.pgdata/` for Nix)

**Questions?** See:
- [NIX_SETUP.md](../guides/NIX_SETUP.md) - Nix-specific database setup
- [ENVIRONMENT_COMPARISON.md](../guides/ENVIRONMENT_COMPARISON.md) - Environment differences
- [CI_ENVIRONMENT.md](./CI_ENVIRONMENT.md) - CI/CD database setup
