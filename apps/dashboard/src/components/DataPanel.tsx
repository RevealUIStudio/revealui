								...
								<thead>
									<tr className="border-b border-gray-600">
										{result.data.headers.map((header: string, i: number) => (
											<th
												key={i}
												className="text-left text-gray-300 font-medium py-2 px-3"
											>
												{header}
											</th>
										))}
						...
						<div className="h-48 bg-gray-700 rounded flex items-center justify-center">
							<div className="text-center text-gray-400">
								...
								<p className="text-xs mt-1">
									...
									{result.data.datasets[0].data.slice(-1)[0].toLocaleString()}{" "}
									latest
								</p>
							</div>
						</div>
				...
				{results.length === 0 ? (
					<div className="h-full flex items-center justify-center text-gray-500">
						<div className="text-center">
							...
							<p className="text-sm">
								Ask questions about your website data, content performance, or
								user analytics. The AI will provide visualizations and insights.
							</p>
						</div>
					</div>
				) : (
