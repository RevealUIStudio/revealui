...
const REQUIRED_IN_PRODUCTION_FORGE = [
  ...
  // the boot fails with a useful name rather than a generic JWT error. Made
  // optional when ALLOW_UNLICENSED_SELF_HOST_ENV opts into the plain
  // self-host Free-tier path (GAP-436) — see the filter in validateStartup.
  ...FORGE_LICENSE_KEYS,
  'CORS_ORIGIN',
...
export function validateStartup(
  ...
  // GAP-436: an explicit plain-self-host opt-in makes the license material
  // optional in forge mode's required list — REVFORGE-stamped kits never set
  // this flag, so their required list (and hard refusal) is untouched.
  const plainSelfHostOptIn = mode === 'forge' && env[ALLOW_UNLICENSED_SELF_HOST_ENV] === 'true';
