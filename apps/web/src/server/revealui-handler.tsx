import { getRevealUI, logger, serializeLexicalState } from "@revealui/core";
...
async function getReveal(): Promise<Awaited<
	ReturnType<typeof getRevealUI>
> | null> {
	if (!revealuiInstance) {
		try {
	...
	if (page.content) {
		if (typeof page.content === "string") {
			...
			if (contentAsLexical.root && typeof contentAsLexical.root === "object") {
				...
				if (lexicalContent) {
					contentElement = (
						<div className="rich-text-content">{lexicalContent}</div>
					);
				} else {
					// Fallback if serialization fails
					contentElement = (
						<div className="rich-text-content">Content unavailable</div>
					);
				}
			} else {
		...
		page.layout && Array.isArray(page.layout) ? (
			<div className="blocks">
				{page.layout.map((block, index) => {
					...
					const blockKey =
						(block as { id?: string | null })?.id ?? `${blockType}-${index}`;
					return (
...
function renderHTML(
	...
	const safeOgDescription = meta?.ogDescription
		? escapeHtml(meta.ogDescription)
		: "";
 ...
 */
export const revealuiHandler: Get<[], UniversalHandler> =
	() => async (request) => {
		...
		try {
			...
			const meta: PageMetadata = {
				title:
					(page.title ? String(page.title) : page.meta?.title) ||
					"RevealUI Web",
				description:
					page.meta?.description ||
					(page.excerpt ? String(page.excerpt) : undefined),
				ogTitle:
					page.meta?.ogTitle || (page.title ? String(page.title) : undefined),
				ogDescription:
			...
			return new Response(
				renderHTML(htmlContent, meta.title || "RevealUI Web", meta),
				{
					status: 200,
					headers: {
			...
			logger.error("RevealUI Handler Error", {
				errorMessage,
				errorStack:
					process.env.NODE_ENV === "production" ? undefined : errorStack,
				error,
			});
