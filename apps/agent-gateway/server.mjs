    ...
    try {
      ...
      if (payload.provider) {
        try {
          ...
          const providerModule = await import(join(__dirname, '..', '..', 'packages', 'inference-clients', `${providerName}.mjs`))
      ...
      const { runAgent } = await import(join(__dirname, '..', '..', 'packages', 'agent-core', 'runAgent.mjs'))

      const result = await runAgent({ id: crypto.randomUUID?.() || Date.now().toString(), mode: 'execute', input: prompt, messages })
