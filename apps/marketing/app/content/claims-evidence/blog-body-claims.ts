...
export const blogBodyClaims: readonly ClaimEntry[] = [
  ...
  },
  {
    file: 'blog/claim-drift',
    exportPath: 'body.11',
    text: 'That hard failure is the whole point. The numbers you read here are not a snapshot someone updated when they remembered. They are a measurement of the code as it exists right now: 32 packages, 66 UI components, 13 first-party MCP servers, 105 database tables, 60 access-control enforcement tests, 5 starter templates. Each one is checked on the commit that publishes it.',
    evidence: [
  ...
  },
  {
    file: 'blog/own-your-data',
    exportPath: 'body.3',
    text: 'RevealUI runs on Postgres. Specifically NeonDB, which is Postgres, the real thing, with the standard wire protocol and `pg_dump` that does exactly what you expect. The schema is 105 tables defined with Drizzle ORM, typed end to end, and it is not hiding any vendor-only behavior in the hot path.',
    evidence: [
  ...
  },
  {
    file: 'blog/why-we-built-revealui',
    exportPath: 'body.51',
    text: '**105 database tables** via Drizzle ORM on NeonDB (Postgres)',
    evidence: [
      {
        kind: 'code',
        ref: 'docs/blog/01-why-we-built-revealui.md',
