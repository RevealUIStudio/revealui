...
export const blogBodyClaims: readonly ClaimEntry[] = [
  ...
  },
  {
    ...
    exportPath: 'body.0',
    text: 'Marketing numbers rot. A landing page says "68 components," the team ships four more, and now the page is wrong and nobody notices, because the page and the code live in different worlds and no one is paid to keep them in sync.',
    evidence: [
  ...
  },
  {
    file: 'blog/claim-drift',
    exportPath: 'body.11',
    text: 'That hard failure is the whole point. The numbers you read here are not a snapshot someone updated when they remembered. They are a measurement of the code as it exists right now: 33 packages, 68 UI components, 14 first-party MCP servers, 123 database tables, 60 access-control enforcement tests, 5 starter templates. Each one is checked on the commit that publishes it.',
    evidence: [
  ...
  },
  {
    file: 'blog/component-library',
    exportPath: 'body.1',
    text: "RevealUI's UI layer, `@revealui/presentation`, has exactly one third-party runtime dependency. Not one UI framework. One npm package: `tailwind-merge`. Its design tokens come from a sibling in-house package, `@revealui/tokens`. Everything else, the 68 components and the machinery that powers them, is in the box and MIT licensed.",
    evidence: [
  ...
  },
  {
    file: 'blog/component-library',
    exportPath: 'body.7',
    text: '`@revealui/presentation` is 68 native React components. Not wrappers around another library. Components, built directly on Tailwind v4 and React.',
    evidence: [
  ...
  },
  {
    file: 'blog/component-library',
    exportPath: 'body.28',
    text: "Colors are not baked into the components. They resolve through a semantic design-token layer in `tokens.css`: `bg-background`, `text-foreground`, `border-border`, `text-primary`, and so on. The tokens carry RevealUI's cobalt palette and adapt to light and dark. Retheme the whole system by changing the token values in one place; you do not touch 68 component files to change your brand color.",
    evidence: [
  ...
  },
  {
    file: 'blog/own-your-data',
    exportPath: 'body.3',
    text: 'RevealUI runs on Postgres. Specifically NeonDB, which is Postgres, the real thing, with the standard wire protocol and `pg_dump` that does exactly what you expect. The schema is 123 tables defined with Drizzle ORM, typed end to end, and it is not hiding any vendor-only behavior in the hot path.',
    evidence: [
  ...
  },
  {
    file: 'blog/why-we-built-revealui',
    exportPath: 'body.31',
    text: 'Free tier gets the full runtime engine, auth, the REST API, and local AI inference (Inference Snaps or Ollama, no API key, no cloud bill). Pro unlocks AI agents, AI memory, signed audit receipts, payments, sync, MCP, and the monitoring dashboard. Max adds unattended inference and higher limits. Enterprise is scoped with sales: unlimited sites and users, plus Enterprise SSO (OIDC + SAML SP-initiated; operator guide [FORGE_SSO_SETUP](../FORGE_SSO_SETUP.md); tracker [#449](https://github.com/RevealUIStudio/revealui/issues/449)).',
    evidence: [
      {
        kind: 'code',
        ref: 'docs/blog/01-why-we-built-revealui.md',
  ...
  },
  {
    file: 'blog/why-we-built-revealui',
    exportPath: 'body.51',
    text: '**123 database tables** via Drizzle ORM on NeonDB (Postgres)',
    evidence: [
      {
        kind: 'code',
        ref: 'docs/blog/01-why-we-built-revealui.md',
  ...
  },
  {
    file: 'blog/why-we-built-revealui',
    exportPath: 'body.52',
    text: '**68 UI components** in `@revealui/presentation`, with one third-party runtime dependency (`tailwind-merge`), built directly on Tailwind v4 and React, with `cva` and `cn` vendored in-package',
    evidence: [
      {
        kind: 'code',
        ref: 'docs/blog/01-why-we-built-revealui.md',
