...
function validForgeProdEnv(overrides: EnvMap = {}): EnvMap {
  return {
    ...
    REVEALUI_SECRET: SECRET_32,
    // KEK intentionally NOT included — forge stamping doesn't generate
    // one today, and forge mode shouldn't require it. A separate test
    // below confirms KEK is optional in forge mode.
...
describe('validateStartup — forge mode', () => {
  ...
  it('does NOT require REVEALUI_KEK in forge mode (optional)', () => {
    // KEK isn't generated by stamp.sh today; forge mode boots without it.
    expect(() => validateStartup(validForgeProdEnv())).not.toThrow();
  });

  it('validates KEK format if a forge customer DOES set one', () => {
    // KEK is optional but must be well-formed if present.
