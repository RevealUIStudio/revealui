 ...
 *
 * IMPORTANT: Vitest 4 requires `function` keyword (not arrow functions) in
 * mockImplementation when the mock is used as a constructor (`new`). Arrow
 * functions cannot be constructors in JavaScript.
 */
...
vi.mock('@revealui/ai/llm/client', () => ({
  // Must use `function` keyword — arrow functions cannot be constructors (Vitest 4)
  // biome-ignore lint/suspicious/noExplicitAny: mock constructor
  LLMClient: vi.fn().mockImplementation((cfg: any) => ({ type: 'byok-client', ...cfg })),
}));

vi.mock('@revealui/ai/orchestration/streaming-runtime', () => ({
  // Must use `function` keyword — arrow functions cannot be constructors (Vitest 4)
  StreamingAgentRuntime: vi.fn().mockImplementation(() => ({
...
async function getRuntimeMock() {
  ...
  // mock.instances holds `this` (the raw prototype object) which is NOT the returned value
...
beforeEach(async () => {
  ...
  // Restore working implementations cleared by clearAllMocks.
  // Use `function` keyword — arrow functions cannot be constructors (Vitest 4).
  ...
  // biome-ignore lint/suspicious/noExplicitAny: mock constructor
  vi.mocked(LLMClient).mockImplementation((cfg: any) => ({ type: 'byok-client', ...cfg }));

  const { StreamingAgentRuntime } = await import('@revealui/ai/orchestration/streaming-runtime');
  vi.mocked(StreamingAgentRuntime).mockImplementation(() => ({
    streamTask: vi.fn().mockImplementation(async function* () {
      yield { type: 'done', result: 'ok' };
    }),
  }));
  ...
  vi.mocked(streamSSE).mockImplementation(async (_c, callback) => {
    ...
    try {
      // biome-ignore lint/suspicious/noExplicitAny: mock stream
...
describe('agent-stream — success path (AI modules working)', () => {
  ...
  it('passes workspaceId from request body to the agent instructions', async () => {
    ...
    // biome-ignore lint/suspicious/noExplicitAny: capturing agent argument
  ...
  it('sets task description to the instruction and type to "instruction"', async () => {
    ...
    const runtime = await getRuntimeMock();
    // biome-ignore lint/suspicious/noExplicitAny: capturing task argument
