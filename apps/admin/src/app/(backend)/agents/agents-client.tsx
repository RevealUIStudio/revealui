...
export default function AgentsPageClient({ isHosted }: { isHosted: boolean }) {
  ...
  return (
    <LicenseGate feature="ai">
      <div className="min-h-screen">
        ...
        {/* Panel content */}
        <div className="p-6">
          {tab === 'agents' ? <AgentCardsPanel /> : <McpServersPanel isHosted={isHosted} />}
        </div>
      </div>
    </LicenseGate>
  );
...
function McpServersPanel({ isHosted }: { isHosted: boolean }) {
  ...
  // GAP-360 §5.7: a hosted customer cannot run a local CLI, so the CLI hint and
  // the CLI empty-state action only render on self-hosted deployments. Hosted
  // copy states honestly that MCP integrations are configured with the
  // deployment (copy-voice: full sentences, no dashes, plain words).
  return (
    <div>
      ...
      {servers.length === 0 ? (
        isHosted ? (
          <EmptyState
            ...
            title="No MCP servers enabled"
            description="MCP integrations are configured with your RevealUI deployment, and this deployment has none enabled yet. Write to support@revealui.com and tell us which integrations you need."
          />
        ) : (
          <EmptyState
            icon={<IconTerminal className="size-6" aria-hidden="true" />}
