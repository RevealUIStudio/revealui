...
>(name: string) {
	...
	const [createCollectionContext, createCollectionScope] =
		createContextScope(ProviderName);
	...
	const [CollectionProviderImpl, useCollectionContext] =
		createCollectionContext<ContextValue>(ProviderName, {
	...
	>((props, forwardedRef) => {
		...
		React.useEffect(() => {
			...
			return () =>
				void context.itemMap.delete(ref as React.RefObject<ItemElement>);
		});
	...
	function useCollection(scope: string | undefined) {
		...
		const getItems = React.useCallback(() => {
			...
			const orderedNodes = Array.from(
				collectionNode.querySelectorAll(`[${ItemDataAttr}]`),
			);
