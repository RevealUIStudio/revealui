 ...
 */
export function transformPageBlockToSchema(
	block: GeneratedBlock,
): Result<Block, z.ZodError> {
	try {
		...
		if (blockType === "cta" && "links" in block) {
			...
			const buttonBlock: ButtonBlock = {
				...
				data: {
					...
					href:
						firstLink.link.url ||
						firstLink.link.reference?.value?.toString() ||
						undefined,
					action: "link",
					variant:
						firstLink.link.appearance === "outline" ? "outline" : "primary",
					size: "md",
				},
				meta: {
					version: 1,
 ...
 */
export function validateAndTransformBlocks(
	blocks: Page["layout"],
): Result<Block[], z.ZodError> {
	...
	switch (block.type) {
				...
				links: [
					{
						link: {
							...
							url: block.data.href,
							appearance:
								block.data.variant === "outline" ? "outline" : "default",
						},
					},
				],
			} as GeneratedBlock;
		...
		case "code": {
			return {
				...
				language: block.data.language as
					| "typescript"
					| "javascript"
					| "css"
					| null,
