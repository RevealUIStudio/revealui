...
function isBlockType<T extends BlockProps>(
	block: BlockProps,
	blockType: string,
): block is T {
...
function normalizeArchiveBlockProps(
	...
	const normalized = {
		...
		categories: block.categories?.map((cat) =>
			typeof cat === "number" ? String(cat) : cat,
		),
...
export const RenderBlocks: React.FC<{
	...
	if (
		...
		logger.warn("Block validation errors (non-strict mode)", {
			adapterErrors: validationResult.success
				? null
			...
			{blocks.map((block, index) => {
				...
				const renderBlock = (): React.ReactNode => {
					try {
						switch (blockType) {
							case "archive": {
								if (!isBlockType<ArchiveBlockProps>(block, "archive"))
									return null;
