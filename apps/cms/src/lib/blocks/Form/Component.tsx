 ...
 * ```
 */
export const FormBlock: React.FC<Props> = memo(
	({ enableIntro, form, introContent }) => {
			...
			async (data: FormData) => {
				...
				const submitForm = async () => {
					...
					try {
						...
						setError({
							message:
								"Form validation failed. Please check your form configuration.",
							status: "400",
						});
						if (process.env.NODE_ENV === "development") {
					...
					<FormProvider {...formMethods}>
						{enableIntro && introContent && !hasSubmitted && (
							<RichText
								className="mb-8"
								content={
									Array.isArray(introContent) ? introContent[0] : introContent
								}
								enableGutter={false}
							/>
						)}
