...
function docsCopyPlugin() {
	...
	return {
		...
		async configureServer(server) {
			...
			server.watcher.on("change", (file) =>
				handleFileOperation(file, "change"),
			);
			server.watcher.on("add", (file) => handleFileOperation(file, "add"));
			server.watcher.on("unlink", (file) =>
				handleFileOperation(file, "unlink"),
			);
			server.watcher.on("unlinkDir", (file) =>
				handleFileOperation(file, "unlinkDir"),
			);
		},
	};
	...
	async function processPendingOperations() {
		...
		console.log(
			`[docs-copy] Processing ${operations.length} file operation(s)...`,
		);
	...
	async function copySingleFile(filePath: string) {
		...
		if (relativePath.startsWith("..") || path.isAbsolute(relativePath)) {
			console.warn(
				`[docs-copy] Skipping file outside docs directory: ${normalizedFile}`,
			);
			return;
		}
