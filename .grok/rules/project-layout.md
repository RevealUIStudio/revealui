...
`~/.claude/rules/git.md` + revcon `profiles/revealfleet/claude/rules/git.md`.
Launch: `rfg` (Plane C). Architecture ADR:
`2026-07-21-harness-policy-runtime-launch-planes`.

## Worktree isolation (HARDLINE)

Multi-step edits: `git worktree add ~/revealfleet/.wt/<name> -b <branch> origin/test`.
Do not use the shared `~/revealfleet/revealui` checkout for concurrent claims.
