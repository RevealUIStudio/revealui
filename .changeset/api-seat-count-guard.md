...
- **`apps/server/src/lib/tier-limits.ts` extraction.** Planned move of `getHostedLimitsForTier` out of `webhooks.ts` into a shared module; the guard takes `maxUsers` as a parameter so the extraction isn't a blocker. Deferred to reduce contention with in-flight edits on `webhooks.ts`.
