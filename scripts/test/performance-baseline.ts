...
async function runAutocannonTest(
	...
	try {
		...
		if (endpointConfig.headers) {
			Object.entries(endpointConfig.headers).forEach(
...
async function main() {
	...
	if (!isHealthy) {
		logger.error(
			"API is not healthy. Please start the CMS server and ensure it is accessible.",
		);
	...
	if (!existsSync(endpointsFile)) {
		...
		logger.info(
			"Please ensure endpoints.json exists in packages/test/load-tests/",
		);
		process.exit(1);
	}
	...
	logger.info(
		`Loaded ${Object.keys(endpointsConfig).length} endpoint configurations`,
	);
	...
	logger.success(
		`Baseline saved to ${baselineFile} with ${results.length} test results`,
	);
