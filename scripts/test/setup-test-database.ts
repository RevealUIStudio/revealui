...
async function setupNeonDatabase(): Promise<DatabaseSetup | null> {
	...
	try {
		...
		logger.warn(
			"Neon API integration not fully implemented, falling back to Docker",
		);
		return null;
	} catch (error) {
		logger.warn(
			`Neon API error: ${error instanceof Error ? error.message : String(error)}`,
		);
		return null;
	}
}
...
export async function setupTestDatabase(): Promise<DatabaseSetup> {
	...
	if (existingUrl) {
		logger.info(
			`Using existing database: ${existingUrl.replace(/:[^:@]+@/, ":****@")}`,
		);
		return {
