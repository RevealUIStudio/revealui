...
function createConsolidationPlan(): ConsolidationPlan {
  ...
  const verifiedClaims: VerifiedClaims = JSON.parse(fs.readFileSync('docs/verified-claims.json', 'utf8'))
  ...
  Object.entries(plan.newStructure).forEach(([section, items]) => {
    console.log(`   ${section}: ${Array.isArray(items) ? items.length : Object.keys(items).length} items`)
  })
...
function categorizeFiles(verifiedClaims: VerifiedClaims['verified']): {
  ...
  filesWithIssues.forEach(file => {
    ...
    else if (file.includes('archive/') && file.includes('duplicate') ||
             file.includes('old/') ||
