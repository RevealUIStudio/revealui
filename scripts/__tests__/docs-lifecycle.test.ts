...
describe('Documentation Lifecycle Manager', () => {
  ...
  describe('File Discovery', () => {
    it('should ignore node_modules', async () => {
      ...
      await fs.mkdir(path.join(testDir, 'node_modules', 'test'), { recursive: true })
      await fs.writeFile(path.join(testDir, 'node_modules', 'test', 'readme.md'), '# Test')
    ...
    it('should handle duplicate archive file names', async () => {
      ...
      while (true) {
        try {
          ...
          finalPath = path.join(archiveDir, `${today}_${base}_${counter}${ext}`)
          counter++
        } catch {
          break
        }
