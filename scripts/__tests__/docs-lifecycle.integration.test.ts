...
describe('Documentation Lifecycle Manager - Integration Tests', () => {
  ...
  beforeEach(async () => {
    testDir = await fs.mkdtemp(path.join(tmpdir(), 'docs-lifecycle-integration-'))
...
`,
      ...
      try {
        ...
        const result = await validateContent(fileDoc!, testConfig)
        expect(result.errors.some((e) => e.type === 'fileReference')).toBe(true)
        expect(result.errors.filter((e) => e.type === 'fileReference').length).toBe(2)
      } finally {
        process.chdir(originalCwd)
      }
    })
...
`,
    ...
    it('should handle duplicate archive file names', async () => {
      ...
      try {
        ...
        const duplicateFileDocs = files.filter((f) => {
          ...
          return normalizedPath === normalizedFile1 || normalizedPath === normalizedFile2
        })
        ...
        const finalArchivedFiles = archiveOutputs[archiveOutputs.length - 1]?.after || []
        const duplicateFiles = finalArchivedFiles.filter((f) => f.includes('duplicate.md'))

        // Also check by reading directory directly
        const archivedFiles = await fs.readdir(archiveDir)
        const actualArchivedFiles = archivedFiles.filter((f) => f !== '.index.json')
        ...
        const duplicateFilesFromDir = actualArchivedFiles.filter((f) => f.includes('duplicate'))
        ...
        if (duplicateFiles.length !== 2 || duplicateFilesFromDir.length !== 2) {
          ...
          console.error('Archive directory contents (from progression):', finalArchivedFiles)
          console.error('Duplicate files found (from progression):', duplicateFiles)
          console.error('Duplicate files found (from directory):', duplicateFilesFromDir)
          console.error('Archive progression:', JSON.stringify(archiveOutputs, null, 2))
        }
  ...
  describe('End-to-End: Real-World Scenarios', () => {
    it('should handle a complete workflow: discover, validate, archive', async () => {
      ...
      const files = [
        { path: path.join(docsDir, 'valid.md'), content: '# Valid\nUses @revealui/core' },
        {
      ...
      try {
        ...
        for (const staleFile of staleFiles) {
          try {
            ...
            expect.fail(`File ${staleFile.file.path} should have been archived`)
          } catch {
