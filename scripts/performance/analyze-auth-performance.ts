...
function analyzePerformance(metrics: PerformanceMetrics[]): {
	...
	for (const metric of metrics) {
		...
		if (metric.p95 > 2000) {
			bottlenecks.push(
				`${endpoint}: p95 response time (${metric.p95}ms) exceeds 2s`,
			);
			score -= 10;
		...
		if (metric.p99 > 5000) {
			bottlenecks.push(
				`${endpoint}: p99 response time (${metric.p99}ms) exceeds 5s`,
			);
			score -= 15;
			recommendations.push(
		...
		if (metric.errorRate > 0.01) {
			...
			score -= 20;
			recommendations.push(
				`Fix errors in ${endpoint} - check logs for root cause`,
			);
		}
		...
		if (
			...
			metric.throughput < targets.signIn.throughput
		) {
			bottlenecks.push(
...
function main() {
	...
	console.log(
		"   Then run: tsx scripts/performance/analyze-auth-performance.ts",
	);
}
