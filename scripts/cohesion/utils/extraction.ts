...
import { readFile } from "../../shared/utils.js";
import type { CodeLocation, PatternInstance } from "../types.js";
...
export async function extractCodeLocation(
	...
	try {
		...
		const before = lines
			.slice(Math.max(0, line - 1 - contextLines), line - 1)
			.map((l) => l.trim());
		const after = lines
		...
		return {
			...
			context:
				before.length > 0 || after.length > 0 ? { before, after } : undefined,
		};
	} catch (_error) {
...
export async function patternInstanceToCodeLocation(
	...
	const location = await extractCodeLocation(
		instance.file,
		instance.line,
		contextLines,
	);
