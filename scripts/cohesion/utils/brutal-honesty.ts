...
export function validateBrutalHonesty(assessmentText: string): {
	...
	const hasBluntLanguage = BRUTAL_HONESTY_RULES.bluntLanguagePatterns.some(
		(pattern) => pattern.test(assessmentText),
	);
	...
	const hasQuantitative =
		/(\d+)\s+(files|instances|issues|lines|percent|%)/i.test(assessmentText);
	...
	const hasCodeExamples =
		/```\d+:\d+:[^\n]+/i.test(assessmentText) ||
		/`[^`]+:\d+`/i.test(assessmentText);
	...
	const hasSeverity =
		/(CRITICAL|HIGH|MEDIUM|LOW)\s+(severity|priority|issue)/i.test(
			assessmentText,
		);
	...
	// Check for grade
	const hasGrade =
		/(Grade|Overall Grade|Assessment Grade):\s*[A-F][+-]?|(D\+|C-|B\+|A-)/i.test(
			assessmentText,
		);
	...
	const hasWouldIUse = /would i (use|recommend|choose)|would you use/i.test(
		assessmentText,
	);
	...
	if (!hasWouldIUse) {
		...
		suggestions.push(
			"Include honest assessment of whether you would use this in production",
		);
	}

	// Check for root cause identification
	const hasRootCause =
		/(root cause|why|because|the problem is|the issue is)/i.test(
			assessmentText,
		);
	...
	const hasRequiredPhrases = BRUTAL_HONESTY_RULES.requiredPhrases.some(
		(pattern) => pattern.test(assessmentText),
	);
	if (!hasRequiredPhrases) {
		violations.push("Missing required brutal honesty phrases");
		suggestions.push(
			'Include phrases like "Bottom line", "The truth is", "Would I use this"',
		);
	}
	...
	const valid =
		Object.values(criteria).filter((v) => v === true).length >= 6 &&
		score >= 70;
...
function calculateBrutalHonestyScore(criteria: BrutalHonestyCriteria): number {
	...
	if (criteria.includesQuantitativeEvidence)
		score += weights.includesQuantitativeEvidence;
...
export function enhanceWithBrutalHonesty(assessmentText: string): {
	...
	if (!BRUTAL_HONESTY_RULES.requiredPhrases.some((p) => p.test(enhanced))) {
		...
		if (!enhanced.includes("Bottom Line")) {
			const summaryMatch = enhanced.match(
				/## Executive Summary\n\n([\s\S]*?)(?=\n##)/,
			);
			if (summaryMatch) {
				const summary = summaryMatch[1];
				if (
					!summary.includes("Bottom Line") &&
					!summary.includes("Bottom line")
				) {
	...
	const replacements: Array<[RegExp, string, string]> = [
		...
		[
			/minor issues/gi,
			"critical issues",
			"Replaced euphemism with direct language",
		],
	];
...
export function validateAnalysisForBrutalHonesty(analysis: CohesionAnalysis): {
	...
	if (issuesWithoutSeverity.length > 0) {
		issues.push(
			`${issuesWithoutSeverity.length} issues missing severity ratings`,
		);
	}
