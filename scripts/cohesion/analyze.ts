...
import { patternInstanceToCodeLocation } from "./utils/extraction.js";
import { calculateGrade, generateMetrics } from "./utils/metrics.js";
import {
	analyzePattern,
	COMMON_PATTERNS,
	findSourceFiles,
 ...
 */
async function patternAnalysisToIssue(
	analysis: PatternAnalysis,
): Promise<CohesionIssue> {
	const evidence = await Promise.all(
		analysis.instances
			.slice(0, 10)
			.map((instance) => patternInstanceToCodeLocation(instance)),
	);
...
async function analyze(): Promise<CohesionAnalysis> {
	...
	const issues = await Promise.all(
		analyses.map((a) => patternAnalysisToIssue(a)),
	);
