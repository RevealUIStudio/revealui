...
import type { RalphState } from "../ralph/types.js";
import {
	checkCompletion,
	isWorkflowActive,
	readStateFile,
...
import { validateBrutalHonesty } from "./utils/brutal-honesty.js";
...
async function cohesionWorkflow(): Promise<void> {
	...
	logger.header(
		`Cohesion Engine - Ralph Workflow (Iteration ${state.iteration})`,
	);
	...
	try {
		...
		if (!cohesionState.analysis_complete) {
			...
			await writeFile(
				cohesionStatePath,
				JSON.stringify(cohesionState, null, 2),
			);
		...
		if (cohesionState.analysis_complete && !cohesionState.assessment_complete) {
			...
			if (await fileExists(assessmentPath)) {
				...
				if (validation.valid) {
					logger.success(
						`Brutal honesty validation passed (${validation.score}/100)`,
					);
				} else {
					logger.warning(
						`Brutal honesty validation failed (${validation.score}/100)`,
					);
...
async function showStatus(): Promise<void> {
	...
	logger.info(`Iteration: ${state.iteration}`);
	logger.info(
		`Analysis Complete: ${cohesionState.analysis_complete ? "✅" : "❌"}`,
	);
	logger.info(
		`Assessment Complete: ${cohesionState.assessment_complete ? "✅" : "❌"}`,
	);
