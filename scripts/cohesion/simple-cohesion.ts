...
async function analyzeFile(filePath: string): Promise<{
	...
	try {
		...
		const complexity = Math.max(
			1,
			functions + classes + imports / 2 + exports / 2,
		);
		...
		if (imports > 15) {
			issues.push({
				...
				suggestion:
					"Consider consolidating imports or breaking down dependencies",
			});
		}
		...
		if (exports === 0 && functions > 0) {
			issues.push({
				...
				severity: "low",
				message:
					"File has functions but no exports - may indicate private utilities",
				suggestion:
					"Consider if this code should be exported or moved to a utility module",
			});
		}
