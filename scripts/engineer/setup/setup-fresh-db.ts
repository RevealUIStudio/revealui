...
import {createLogger,validateDependencies} from '../../../packages/core/src/.scripts/utils.ts'
  ...
  try {
    ...
    try {
      ...
      const passwordHashMigrationPath = path.resolve(
        __dirname,
        '../../packages/db/src/orm/drizzle/0001_add_password_hash.sql',
      )
      ...
      const emailUniqueMigrationPath = path.resolve(
        __dirname,
        '../../packages/db/src/orm/drizzle/0002_add_email_unique_constraint.sql',
      )
