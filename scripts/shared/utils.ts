...
export function createLogger(): Logger {
  ...
  const forceColor = process.env.FORCE_COLOR !== '0' && process.env.FORCE_COLOR !== 'false'
  const hasTerm = process.env.TERM && process.env.TERM !== 'dumb'
  const isTTY = process.stdout.isTTY
  const supportsColor = forceColor || (isTTY && (hasTerm || process.platform === 'win32'))
...
export async function execCommand(
  ...
  return new Promise((resolve) => {
    const stdioConfig: ('pipe' | 'inherit' | 'ignore')[] = options.stdin
      ? ['pipe', options.silent ? 'pipe' : 'inherit', options.silent ? 'pipe' : 'inherit']
      : options.silent
        ? ['ignore', 'pipe', 'pipe']
