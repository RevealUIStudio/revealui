...
import { fileExists, readFile, writeFile } from "../shared/utils.js";
...
export async function readStateFile(
	...
	if (!frontmatterMatch) {
		throw new Error(
			`Invalid state file format: ${stateFilePath}\nMissing YAML frontmatter.`,
		);
	}
	...
	const state: RalphState = {
		...
		completion_marker:
			(frontmatter.completion_marker as string) || COMPLETION_MARKER_PATH,
	};
 ...
 */
export async function readCompletionMarker(
	projectRoot: string,
): Promise<string | null> {
