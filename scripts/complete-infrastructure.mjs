...
`;
  ...
  execSync('git commit -m "test: validate complete CI/CD infrastructure"', { stdio: 'inherit' });
...
function main() {
  ...
  switch (command) {
    ...
    case 'run':
      ...
      if (!taskName) {
        console.error('❌ Please specify a task name: pnpm complete:infrastructure run <task>');
        process.exit(1);
      }
