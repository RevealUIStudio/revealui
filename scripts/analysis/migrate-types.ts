...
async function migrateFile(
				...
				if (ts.isStringLiteral(moduleSpecifier)) {
					...
					if (
						OLD_IMPORT_SOURCES.some(
							(oldSource) =>
								source === oldSource || source.startsWith(oldSource + "/"),
						)
					) {
								...
								for (const type of types) {
									...
									if (migration) {
										if (migration.deprecated && config.addDeprecations) {
											...
											result.changes.push(
												`DEPRECATION: ${type} - ${migration.deprecationMessage}`,
											);
										}
	...
	if (schemaImports.size > 0 && config.rewriteImports) {
		...
		if (schemaImportLineIndex >= 0) {
			...
			result.changes.push(
				`MERGE: Added types to existing @revealui/contracts/cms import`,
			);
			...
			const firstImportIndex = newLines.findIndex((l) =>
				l.startsWith("import"),
			);
...
async function runMigration() {
	...
	try {
		...
		logger.info(
			`Options: deprecate=${config.addDeprecations}, rewrite=${config.rewriteImports}`,
		);
		logger.info("");
		...
		if (!config.addDeprecations && !config.rewriteImports) {
			...
			logger.info("Usage:");
			logger.info(
				"  pnpm tsx scripts/analysis/migrate-types.ts --dry-run --deprecate",
			);
			logger.info(
				"  pnpm tsx scripts/analysis/migrate-types.ts --dry-run --rewrite",
			);
			logger.info(
				"  pnpm tsx scripts/analysis/migrate-types.ts --rewrite  # Execute migration",
			);
			process.exit(0);
		}
