...
async function generateReport(
	results: Array<{ name: string; code: number; output?: string }>,
) {
...
async function runQualityChecks(options: QualityOptions) {
	...
	if (!options.ci || options.strict) {
		const tscResult = await runQualityCheck(
			"pnpm",
			["dlx", "tsc", "--noEmit"],
			{
...
async function main() {
	try {
		...
		logger.error(
			`Quality script failed: ${error instanceof Error ? error.message : String(error)}`,
		);
