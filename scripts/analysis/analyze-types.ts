...
function extractImportedTypes(
	specifiers: ts.NodeArray<ts.ImportSpecifier>,
): string[] {
	const types: string[] = [];
		...
		for (const typeArg of node.typeArguments) {
			if (ts.isTypeReferenceNode(typeArg) && isTargetType(typeArg.typeName)) {
				...
				usages.push({
					...
					file: filePath,
					line:
						context.sourceFile.getLineAndCharacterOfPosition(typeArg.getStart())
							.line + 1,
					context: "generic-param",
					snippet: lineText.substring(0, 100),
					importSource: importedTypes.get(typeName),
		...
		ts.forEachChild(sourceFile, (node) => {
			if (ts.isImportDeclaration(node)) {
				...
				if (ts.isStringLiteral(moduleSpecifier)) {
					...
					if (
						IMPORT_SOURCES.some(
							(s) => source === s || source.startsWith(s + "/"),
						)
					) {
						if (node.importClause) {
							...
							if (namedImports && ts.isNamedImports(namedImports)) {
								...
								const targetTypes = types.filter((t) =>
									TARGET_TYPES.includes(t),
								);
...
function calculateComplexity(
	...
	if (type.includes("Hook")) {
		return {
			complexity: "high",
			reason:
				"Hook types require TypeScript generic signatures, cannot use Zod",
		};
	}
...
async function runAnalysis() {
	try {
		...
		for (const file of files) {
			try {
				...
				for (const usage of analysis.usages) {
					...
					if (
						usage.importSource &&
						!report.byType[usage.type].importSources.includes(
							usage.importSource,
						)
					) {
		...
		for (const [type, data] of Object.entries(report.byType)) {
			const { complexity, reason } = calculateComplexity(
				type,
				data.totalUsages,
				data.files.length,
			);
		...
		const hookTypes = report.migrationPriority.filter((p) =>
			p.type.includes("Hook"),
		);
		const revealTypes = report.migrationPriority.filter((p) =>
			p.type.startsWith("Reveal"),
		);
		...
		if (report.migrationPriority.length > 15) {
			logger.info(
				`   ... and ${report.migrationPriority.length - 15} more types`,
			);
		}
		logger.info("");
