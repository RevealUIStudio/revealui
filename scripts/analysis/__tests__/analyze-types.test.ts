								...
								if (ts.isExpressionWithTypeArguments(type)) {
									...
									if (ts.isIdentifier(expr) || ts.isQualifiedName(expr)) {
										if (isTargetType(expr)) {
											const typeName = ts.isIdentifier(expr)
												? expr.text
												: expr.right.text;
											usages.push(typeName);
										}
					...
					for (const typeArg of node.typeArguments) {
						if (ts.isTypeReferenceNode(typeArg)) {
							if (isTargetType(typeArg.typeName)) {
								const typeName = ts.isIdentifier(typeArg.typeName)
...
const arr: Array<CollectionConfig> = []`;
			...
			const contexts: Array<
				"import" | "type-annotation" | "interface-extends" | "generic-param"
			> = [];
