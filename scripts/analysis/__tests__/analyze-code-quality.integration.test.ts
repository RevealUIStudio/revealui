...
export function func2(): void {
  // FIXME: implement this
		...
		const file1Result = summary.results.find((r) =>
			r.file.includes("file1.ts"),
		);
		const file2Result = summary.results.find((r) =>
			r.file.includes("file2.ts"),
		);
...
const y: any = 2`,
		...
		expect(
			summary.results[0].todos + summary.results[0].anyTypes,
