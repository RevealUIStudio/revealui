...
async function verifyRuntime() {
	try {
		...
		try {
			...
			try {
				const main = await importWithFallback(
					"services",
					servicesPath,
			...
			try {
				...
				const core = await importWithFallback(
					"services/core",
					corePath,
			...
			try {
				...
				if (main.protectedStripe !== core.protectedStripe) {
					errors.push(
						"❌ protectedStripe not consistent between main and core",
					);
				} else {
				...
				if (main.createPaymentIntent !== core.createPaymentIntent) {
					errors.push(
						"❌ createPaymentIntent not consistent between main and core",
					);
				} else {
