	...
	if (violations.length > 0) {
		...
		if (options.fix) {
				...
				for (const violation of files) {
					...
					try {
						...
						if (existsSync(targetPath)) {
							logger.warning(
								`  ⚠️  ${violation.file} already exists in ${subfolder}/, skipping`,
							);
							continue;
						}
						...
						moved++;
						const backupPath = backupCreated
							? join(backupDir, violation.file)
							: undefined;
						movedFiles.push({
							source: sourcePath,
							target: targetPath,
						...
						logger.success(
							`  ✅ Moved ${violation.file} -> ${subfolder}/${violation.file}`,
						);
					} catch (error) {
						failed++;
						logger.error(
			...
			if (moved > 0 && backupCreated) {
				...
				logger.info(
					`📦 Rollback info saved to: ${backupDir}/rollback-info.json`,
				);
		...
		logger.success(
			"✅ No violations found! All root markdown files are allowed.",
		);
	}
