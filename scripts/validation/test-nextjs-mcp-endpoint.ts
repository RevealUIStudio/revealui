...
async function checkServer(port: number): Promise<{ running: boolean; statusCode?: number }> {
  try {
...
async function checkMCPEndpoint(): Promise<{ accessible: boolean; content?: string }> {
  try {
...
async function testMCPEndpoint() {
  ...
  if (
    ...
    if (mcpStatus.accessible) {
      ...
      if (mcpStatus.content) {
        ...
        if (mcpStatus.content.length > preview.length) {
          logger.info(`   ... (${mcpStatus.content.length - preview.length} more characters)`)
        }
      }
    } else {
    ...
    logger.error(`   Server not responding correctly (HTTP ${serverStatus.statusCode})`)
  } else {
