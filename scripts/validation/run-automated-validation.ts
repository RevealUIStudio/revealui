...
async function checkDocker() {
	...
	if (!hasDocker) {
		logger.error(
			"Docker is not installed. Please install Docker to run automated tests.",
		);
		process.exit(1);
	}
}
...
async function setupTestDatabase(projectRoot: string) {
	...
	const result = await execCommand(
		"pnpm",
		["tsx", "scripts/database/setup-test-db.ts"],
		{
			cwd: projectRoot,
		},
	);
...
async function runPerformanceTests(projectRoot: string) {
	...
	const perfScript = join(
		projectRoot,
		"scripts/analysis/measure-performance.ts",
	);
