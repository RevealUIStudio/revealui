...
async function checkDatabaseConnection(
	...
	if (postgresUrl.includes("neon.tech") || postgresUrl.includes("neon")) {
		...
		recordInfo(
			"Neon HTTP driver requires Neon instance - local PostgreSQL won't work",
		);
	}
	...
	const testScript = join(
		projectRoot,
		"scripts/database/test-neon-connection.mjs",
	);
	...
	if (await fileExists(testScript)) {
		...
		recordInfo(
			"Connection test script not found - will validate via integration tests",
		);
	}
}
...
async function runIntegrationTests(postgresUrl: string, projectRoot: string) {
	...
	const testResult = await execCommand(
		...
		[
			...
			"@revealui/ai",
			"test",
			"__tests__/integration/automated-validation.test.ts",
		],
		{
			cwd: projectRoot,
			env: { POSTGRES_URL: postgresUrl },
	...
	if (perfMatches && perfMatches.length > 0) {
		...
		if (timeMatch) {
			...
			if (avgTime < threshold) {
				recordSuccess(
					`Performance within acceptable range (${avgTime}ms < ${threshold}ms)`,
				);
			} else {
				recordWarning(
					`Performance may be slow (${avgTime}ms) - expected < ${threshold}ms`,
				);
		...
		recordInfo(
			"Performance tests may be in separate test file or not included",
		);
...
async function runValidation() {
	...
	recordWarning(
		"Note: Run 'pnpm --filter @revealui/db db:push' if migrations are needed",
	);
