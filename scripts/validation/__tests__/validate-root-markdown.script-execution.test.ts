...
import { validateRootMarkdown } from "../validate-root-markdown.js";

const __dirname = fileURLToPath(new URL(".", import.meta.url));
const testProjectRoot = join(
	__dirname,
	"../../..",
	".test-validate-root-markdown-script",
);
...
describe("Validate Root Markdown - Script Execution Tests", () => {
	...
	describe("Script Execution - Non-Fix Mode", () => {
		it("should detect violations without fixing them", async () => {
			...
			expect(
				existsSync(join(testProjectRoot, "docs", "agent", "AGENT_HANDOFF.md")),
			).toBe(false);
			expect(
				existsSync(join(testProjectRoot, "docs", "guides", "TEST_GUIDE.md")),
			).toBe(false);
		});
	...
	describe("Script Execution - Fix Mode", () => {
		it("should move violation files to correct subfolders", async () => {
			...
			expect(
				existsSync(
					join(testProjectRoot, "docs", "assessments", "TEST_ASSESSMENT.md"),
				),
			).toBe(true);
		});
		...
		it("should create backup before moving files", async () => {
			...
			const hasBackupDir = backupDirs.some((d) =>
				d.startsWith("markdown-move-"),
			);
		...
		it("should create rollback info after moving files", async () => {
			...
			if (latestBackupDir) {
				...
				const rollbackInfo = JSON.parse(
					await readFile(rollbackInfoPath, "utf-8"),
				);
		...
		it("should skip files that already exist in target", async () => {
			...
			await writeFile(
				join(testProjectRoot, "docs", "EXISTING.md"),
				"# Existing",
			);
			...
			expect(existsSync(join(testProjectRoot, "docs", "EXISTING.md"))).toBe(
				true,
			);
		...
		it("should create nested directories as needed", async () => {
			...
			expect(existsSync(join(testProjectRoot, "docs", "assessments"))).toBe(
				true,
			);
			expect(
				existsSync(
