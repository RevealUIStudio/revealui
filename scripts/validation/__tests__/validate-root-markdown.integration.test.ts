...
import {
	determineTargetSubfolder,
	isAllowedRootFile,
} from "../validate-root-markdown.js";

const __dirname = fileURLToPath(new URL(".", import.meta.url));
const testProjectRoot = join(
	__dirname,
	"../../..",
	".test-validate-root-markdown",
);
...
describe("Validate Root Markdown - Integration Tests", () => {
	...
	describe("File Moving Operations", () => {
		it("should move violation files to correct subfolders", async () => {
			...
			await writeFile(
				join(testProjectRoot, "AGENT_HANDOFF.md"),
				"# Agent Handoff",
			);
			await writeFile(join(testProjectRoot, "TEST_GUIDE.md"), "# Test Guide");
			await writeFile(
				join(testProjectRoot, "TEST_ASSESSMENT.md"),
				"# Test Assessment",
			);

			// Mock getProjectRoot to return test directory
			const _originalGetProjectRoot = await import(
				"../../shared/utils.js"
			).then((m) => m.getProjectRoot);
			...
			expect(existsSync(join(testProjectRoot, "TEST_ASSESSMENT.md"))).toBe(
				true,
			);
			...
			expect(determineTargetSubfolder("TEST_ASSESSMENT.md")).toBe(
				"docs/assessments",
			);
		});
	...
	describe("End-to-End Workflow", () => {
		it("should complete full workflow: create files → categorize → backup → move → verify", async () => {
			...
			for (const categorization of categorizations) {
				expect(
					existsSync(
						join(testProjectRoot, categorization.target, categorization.file),
					),
				).toBe(true);
				expect(existsSync(join(testProjectRoot, categorization.file))).toBe(
					false,
				);
			}
		});
