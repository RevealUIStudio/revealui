...
import { determineTargetSubfolder, isAllowedRootFile } from '../validate-root-markdown.js'
...
describe('Validate Root Markdown - Integration Tests', () => {
  ...
  describe('File Moving Operations', () => {
    it('should move violation files to correct subfolders', async () => {
      ...
      const _originalGetProjectRoot = await import('../../shared/utils.js').then(
        (m) => m.getProjectRoot,
      )
  ...
  describe('End-to-End Workflow', () => {
    it('should complete full workflow: create files → categorize → backup → move → verify', async () => {
      ...
      const testFiles = [
        ...
        { name: 'AGENT_HANDOFF.md', content: '# Agent Handoff', allowed: false },
        { name: 'TEST_GUIDE.md', content: '# Test Guide', allowed: false },
        { name: 'TEST_ASSESSMENT.md', content: '# Test Assessment', allowed: false },
      ]
