...
describe("check-console-statements.ts - Integration Tests", () => {
	...
	beforeEach(async () => {
		...
		await mkdir(join(testDir, "packages", "core", "src", "utils"), {
			recursive: true,
		});
		await mkdir(join(testDir, "packages", "core", "src", "instance"), {
			recursive: true,
		});
  ...
  console.error('file2 console')
		...
		const result = await checkConsoleStatements(
			[
				join(testDir, "packages", "core", "src"),
				join(testDir, "apps", "cms", "src"),
			],
			testDir,
		);
		...
		const file1Matches = result.matches.filter((m) =>
			m.file.includes("file1.ts"),
		);
		const file2Matches = result.matches.filter((m) =>
			m.file.includes("file2.ts"),
		);
  ...
  console.log('This should be detected')
		...
		const loggerMatches = result.matches.filter((m) =>
			m.file.includes("logger.ts"),
		);
		const regularMatches = result.matches.filter((m) =>
			m.file.includes("regular.ts"),
		);
