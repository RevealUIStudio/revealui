...
const testProjectRoot = join(
	__dirname,
	"../../..",
	".test-validate-root-markdown-rollback",
);
...
describe("Validate Root Markdown - Rollback Integration Tests", () => {
	...
	describe("Rollback Info Format", () => {
		it("should create rollback info in correct format", async () => {
			...
			const rollbackInfo: RollbackInfo = JSON.parse(
			...
			expect(new Date(rollbackInfo.timestamp).toISOString()).toBe(
		...
		it("should match format expected by rollback script", async () => {
			...
			for (const movedFile of rollbackInfo.movedFiles) {
				...
				expect(
					movedFile.source.startsWith("/") ||
						movedFile.source.includes(testProjectRoot),
				).toBe(true);
				expect(
					movedFile.target.startsWith("/") ||
						movedFile.target.includes(testProjectRoot),
				).toBe(true);
			}
		});
	});
	...
	describe("End-to-End: Validate → Move → Rollback", () => {
		it("should restore files correctly after rollback", async () => {
			...
			const restoredContent1 = await readFile(
				join(testProjectRoot, "AGENT_HANDOFF.md"),
				"utf-8",
			);
			const restoredContent2 = await readFile(
				join(testProjectRoot, "TEST_GUIDE.md"),
				"utf-8",
			);
