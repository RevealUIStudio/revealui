	...
	try {
		...
		try {
			...
			if (execError && typeof execError === "object") {
				...
				result =
					`${stdout}\n${stderr}`.trim() ||
					String(execError.message || execError);
			} else if (
				execError &&
				...
				logger.error(
					"   This is a command execution failure, not a type error.",
				);
				process.exit(1);
			}
		}
		...
		if (servicesErrors.length > 0) {
			...
			servicesErrors.forEach((error) => logger.error(`  ${error}`));
			process.exit(1);
		}

		// Check for any type errors (not just services-related)
		const allTypeErrors = result
			.split("\n")
