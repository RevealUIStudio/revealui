...
class RootFileCleaner {
  ...
  private async analyzeRootFiles(): Promise<void> {
    ...
    for (const file of rootFiles) {
      if (
        ['README.md', 'LICENSE', 'package.json', 'pnpm-lock.yaml', 'pnpm-workspace.yaml'].includes(
          file,
        )
      ) {
        ...
        file.endsWith('.md') &&
        ['CHANGELOG', 'CONTRIBUTING', 'SECURITY', 'CODE_OF_CONDUCT', 'REVEALUI_FEATURES'].some(
          (prefix) => file.startsWith(prefix),
        )
      ) {
        categories.documentation.push(file)
      } else if (
        ['biome.json', 'env.d.ts', 'renovate.json5', 'vercel.json', 'openapi.json'].includes(file)
      ) {
