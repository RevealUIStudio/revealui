 ...
 * Fails if any .env file (not .env.template/.env.production.example) is tracked by git.
 */
async function checkEnvFiles(projectRoot: string): Promise<CheckResult> {
  ...
  // Local/test files such as .env.test stay untracked (.gitignore .env.*).
  const AllowedEnvFiles = new Set(['.env.template', '.env.production.example']);
