	...
	for (const file of sourceFiles) {
		try {
			const content = await readFile(
				join(await getProjectRoot(import.meta.url), file),
				"utf-8",
			);
			...
			for (const item of exportedItems) {
				...
				if (itemName && !content.includes(`/**`)) {
					...
					const linesBefore = content
						.substring(0, content.indexOf(item))
						.split("\n");
					const hasJSDoc = linesBefore.some((line) =>
		...
		try {
			...
			while ((match = linkRegex.exec(content)) !== null) {
				...
				if (
					...
					try {
						await readFile(
							join(await getProjectRoot(import.meta.url), resolvedPath),
						);
					} catch {
						results.push({
							file,
	...
	try {
		...
		if (results.length === 0) {
			...
			if (summary.errors > 0) {
				...
				logger.warning(
					`⚠️ ${summary.warnings} warnings found - consider fixing`,
				);
			}
		}
	} catch (error) {
