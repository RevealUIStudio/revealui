...
async function checkFileReferenced(
	filePath: string,
	projectRoot: string,
): Promise<boolean> {
	...
	for (const activeFile of activeFiles) {
		try {
			...
			if (verboseMode && error instanceof Error) {
				logger.warning(
					`⚠️  Could not check file ${activeFile}: ${error.message}`,
				);
			}
		}
	}
...
async function reviewArchive(): Promise<ArchiveReview> {
	...
	const review = archiveFiles.filter(
		(f) =>
			!f.canDelete &&
			!f.referenced &&
			f.daysSinceModified < RETENTION_POLICY.default,
	);
...
function generateReportMarkdown(review: ArchiveReview): string {
	...
	if (review.recommendations.safeToDelete.length === 0) {
		...
		for (const [category, files] of byCategory.entries()) {
			...
			for (const file of files.sort(
				(a, b) => b.daysSinceModified - a.daysSinceModified,
			)) {
	...
	if (review.summary.safeToDelete > 0) {
		...
		lines.push(
			"2. **Check again in 3 months** - Files may become eligible for deletion",
		);
	}
	lines.push("");
...
async function main() {
	...
	try {
		...
		if (dryRun) {
			...
			logger.info(
				`   Potential space savings: ${formatBytes(review.summary.deletableSize)}`,
			);

			if (review.summary.safeToDelete > 0) {
				logger.warn(
					`⚠️  [DRY RUN] Would identify ${review.summary.safeToDelete} files for deletion`,
				);
			} else {
