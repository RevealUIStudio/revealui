...
function categorizeRootFile(
	...
	if (
		/^(BRUTAL|ASSESSMENT|AGENT_WORK|PACKAGE_RENAME|TYPE_SYSTEM)/i.test(baseName)
	) {
		return {
	...
	if (
		/^(IMPORT|TYPESCRIPT|RESTART|EXTENSION|CRITICAL|ELECTRIC)/i.test(baseName)
	) {
		return {
...
async function consolidateRootDocs(
	dryRun = false,
	...
	for (const file of filesToMove) {
		try {
			...
			try {
				...
				logger.warning(
					`  ⚠️  Target exists, skipping: ${path.relative(projectRoot, file.target)}`,
				);
				continue;
			} catch {
