...
async function findAssessmentFiles(): Promise<string[]> {
	...
	const packageAssessments = await fg(
		["packages/*/*ASSESSMENT*.md", "packages/*/ASSESSMENT*.md"],
		{
			cwd: projectRoot,
			absolute: true,
		},
	...
	for (const sourceOfTruth of RULES.sourcesOfTruth) {
		...
		try {
			...
			if (verboseMode && error instanceof Error) {
				logger.warning(
					`⚠️  Could not check source of truth ${sourceOfTruth}: ${error.message}`,
				);
			}
		}
	}
...
async function analyzeAssessments(): Promise<AssessmentReport> {
	...
	for (const [packageName, files] of packageGroups.entries()) {
		if (files.length > RULES.maxAssessmentsPerPackage) {
			...
			recommendations.push(
				`Archive or delete excess assessment files in packages/${packageName}/`,
			);
		}
	}

	// Generate recommendations
	const packageAssessments = assessments.filter(
		(a) => a.category === "package",
	);
	...
	const shouldArchive = assessments.filter(
		(a) => a.status === "should-archive",
	);
...
function generateReportMarkdown(report: AssessmentReport): string {
	...
	// Group by status
	const byStatus = {
		"should-archive": report.assessments.filter(
			(a) => a.status === "should-archive",
		),
		"needs-review": report.assessments.filter(
	...
	>) {
		...
		const emoji =
			status === "should-archive"
				? "📦"
				: status === "needs-review"
					? "⚠️"
					: "✅";
		lines.push(
			`### ${emoji} ${status.replace(/-/g, " ").toUpperCase()} (${files.length})`,
		);
		lines.push("");
...
async function main() {
	...
	try {
		...
		if (dryRun) {
			...
			if (report.violations.length > 0) {
				logger.warn(
					`⚠️  [DRY RUN] Would warn about ${report.violations.length} violations`,
				);
			} else {
