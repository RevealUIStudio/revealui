...
async function validateCodeBlock(
	...
	if (code.includes("export default") && code.includes("module.exports")) {
		issues.push({
			...
			suggestion:
				"Use only ESM (export) or CommonJS (module.exports), not both",
		});
	}
...
async function validateImports(
	...
	for (const match of importMatches) {
		...
		if (importPath.startsWith("@revealui/") || importPath.startsWith("dev/")) {
			...
			const packageName = importPath.includes("/")
				? importPath.split("/")[0]
				: importPath;
			const packagePath = path.join(
				projectRoot,
				"packages",
				packageName.replace("@revealui/", ""),
			);
...
async function validateDocumentation(
	...
	const packageIssues = await validatePackageReferences(
		content,
		filePath,
		projectRoot,
	);
	...
	try {
		...
		if (dryRun) {
			...
			if (report.summary.errors > 0) {
				logger.error(
					`❌ [DRY RUN] Would fail with ${report.summary.errors} errors`,
				);
