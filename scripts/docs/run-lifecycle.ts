...
function parseArgs(args: string[]): {
	...
	if (!["weekly", "monthly", "quarterly"].includes(type)) {
		throw new Error(
			`Invalid lifecycle type: ${type}. Must be weekly, monthly, or quarterly.`,
		);
	}
...
async function runLifecycle(
	...
	if (allSuccessful) {
		logger.success(
			`🎉 ${type} documentation lifecycle checks completed successfully!`,
		);
		return true;
	} else {
		logger.error(
			`⚠️  ${type} documentation lifecycle checks completed with failures.`,
		);
		logger.error(
