...
async function maintenanceCheck(): Promise<void> {
	...
	if (!staleResult.success) {
		logger.warning(
			"  ⚠️  Stale documentation detected. Run `pnpm docs:archive` to clean up.",
		);
	} else {
	...
	if (linksResult.success) {
		...
		logger.warning(
			"  ⚠️  Broken links detected. Run `pnpm docs:verify:links` for details.",
		);
	}

	logger.info("\n\nMaintenance check complete!");
	logger.info(
		"💡 Tip: Run `pnpm docs:validate:all` for comprehensive validation",
	);
}
