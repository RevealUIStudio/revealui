...
export async function extractAnchors(
	...
	try {
		...
		if (error instanceof Error) {
			logger.warning(
				`⚠️  Could not read file to extract anchors: ${filePath} - ${error.message}`,
			);
		} else {
			logger.warning(
				`⚠️  Could not read file to extract anchors: ${filePath} - Unknown error`,
			);
		}
	}
...
export async function validateReferences(
	...
	for (const match of linkMatches) {
		...
		const targetResult = await resolveLinkTarget(
			filePathPart,
			filePath,
			projectRoot,
		);
		...
		if (!targetResult.exists) {
			if (targetResult.isArchived) {
				issues.push({
					...
					suggestion:
						"Update link to point to current documentation or remove if obsolete",
				});
			} else {
				issues.push({
					file: path.relative(projectRoot, filePath),
...
export function generateReportMarkdown(report: ReferenceReport): string {
	...
	// Show warnings
	const warnings = report.brokenReferences.filter(
