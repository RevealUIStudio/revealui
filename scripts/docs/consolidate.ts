...
function createConsolidationPlan(): ConsolidationPlan {
	...
	const plan: ConsolidationPlan = {
		summary: {
			...
			filesWithIssues: Object.keys(
				verifiedClaims.verified.reduce(
					(acc: Record<string, boolean>, claim) => {
						acc[claim.file] = true;
						return acc;
					},
					{},
	...
	console.log(
		`   Confirmed false claims: ${plan.summary.confirmedFalseClaims}`,
	);
...
function createActionPlan(
	...
	fileActions.update.forEach((file) => {
		...
		actions.push({
			type: "update",
			file: file,
			priority: claims.some((c) => c.verification.priority === "critical")
				? "critical"
				: "high",
