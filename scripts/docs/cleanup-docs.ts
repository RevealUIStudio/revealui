...
async function cleanupStaleDocs(): Promise<void> {
	...
	logger.info(
		`Cleanup complete: ${archived} archived, ${reviewed} reviewed and kept`,
	);
}
...
async function cleanupDuplicates(): Promise<void> {
	...
	for (const group of duplicates) {
		...
		const keep = await promptUser(
			'Which file to keep? (Enter filename or "skip")',
		);
		if (keep && keep !== "skip") {
...
async function findDuplicates(): Promise<
	Array<{ content: string; files: string[] }>
> {
	const projectRoot = await getProjectRoot(import.meta.url);
	const docsDir = join(projectRoot, "docs");
	const files = await scanDocsDirectory(docsDir);
...
function generateOptimizationSuggestions(
	...
	for (const [category, count] of Object.entries(structure)) {
		if (count === 0) {
			...
			suggestions.push(
				`Consider merging small category: ${category} (${count} files)`,
			);
		}
	}
...
async function promptUser(question: string): Promise<boolean> {
	const answer = await import("../shared/utils.js").then((m) =>
		m.prompt(`${question} (y/N)`),
	);
