...
async function mergeFiles(options: MergeOptions): Promise<void> {
	...
	for (const source of options.sources) {
		...
		try {
			...
			const title =
				extractTitle(content) || path.basename(source, path.extname(source));
...
async function main() {
	...
	if (args.length < 2) {
		...
		logger.info("\nExample:");
		logger.info(
			"  pnpm tsx scripts/docs/merge-docs.ts docs/guides/auth-complete.md docs/AUTH_*.md",
		);
		process.exit(1);
	}
	...
	if (!target || sources.length === 0) {
		logger.error(
			"Error: Target file and at least one source file are required",
		);
		process.exit(1);
	}
