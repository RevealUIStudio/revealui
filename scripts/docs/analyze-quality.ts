...
async function analyzeJSDocCoverage(): Promise<JSDocCoverage> {
	...
	if (undocumented.length > 0) {
		logger.warning(
			`Found ${undocumented.length} files with undocumented exports`,
		);
...
async function analyzeQualityMetrics(): Promise<QualityMetrics> {
	...
	logger.info(
		`Average file size: ${(metrics.avgFileSize / 1024).toFixed(1)} KB`,
	);
	logger.info(
...
async function analyzeTrends(): Promise<void> {
	...
	logger.info(
		"Trend analysis would identify most active documentation areas...",
	);
