...
async function generateLinksReport(result: LinksVerificationResult): Promise<string> {
  const lines: string[] = []
...
async function verifyVersions(files: string[]): Promise<VersionsVerificationResult> {
...
async function generateVersionsReport(result: VersionsVerificationResult): Promise<string> {
  const lines: string[] = []
...
async function verifyCommands(files: string[]): Promise<CommandsVerificationResult> {
...
async function generateCommandsReport(result: CommandsVerificationResult): Promise<string> {
  const lines: string[] = []
...
async function generatePathsReport(result: PathsVerificationResult): Promise<string> {
  const lines: string[] = []
...
async function verifyCodeExamples(files: string[]): Promise<CodeExamplesVerificationResult> {
...
async function generateCodeExamplesReport(result: CodeExamplesVerificationResult): Promise<string> {
  const lines: string[] = []
...
async function runVerifications(modules: string[]): Promise<VerificationRunResult[]> {
  ...
  for (const moduleName of modules) {
    ...
    try {
      ...
      if (result.success) {
        ...
        logger.error(`  ❌ ${moduleName} failed (${result.issues.length} issues)`)
      }
    } catch (error) {
      logger.error(
  ...
  for (const arg of args) {
    if (arg.startsWith('--')) {
      ...
      const module = VERIFICATION_MODULES.find(
        (m) => m.name === moduleName || m.name === moduleName.replace(/-/g, '-'),
      )
      if (module) {
...
async function main() {
  try {
    ...
    logger.info(`Running ${modulesToRun.length} verification(s): ${modulesToRun.join(', ')}\n`)
    ...
    for (const result of results) {
      ...
      if (result.reportPath) {
        logger.info(`   Report: ${path.relative(process.cwd(), result.reportPath)}`)
      }
      if (!result.success) {
        allPassed = false
      }
