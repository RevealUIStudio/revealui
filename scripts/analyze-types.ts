...
function calculateComplexity(type: string, usageCount: number, fileCount: number): {
  complexity: "low" | "medium" | "high";
  reason: string;
} {
  // Hook types are always high complexity (need TypeScript generics)
  if (type.includes("Hook")) {
    return { complexity: "high", reason: "Hook types require TypeScript generic signatures, cannot use Zod" };
  }

  // Access types are medium complexity
  if (type.includes("Access")) {
    return { complexity: "medium", reason: "Access functions need TypeScript types but simpler signatures" };
  }
  ...
  if (usageCount > 20 || fileCount > 10) {
    return { complexity: "high", reason: `High usage count (${usageCount} usages across ${fileCount} files)` };
  }

  if (usageCount > 5 || fileCount > 3) {
    return { complexity: "medium", reason: `Moderate usage (${usageCount} usages across ${fileCount} files)` };
  }
...
async function main() {
  console.log("🔍 Type Usage Analyzer for CMS Types Migration\n");
  ...
  for (const file of files) {
    try {
      ...
      for (const usage of analysis.usages) {
        ...
        if (usage.importSource && !report.byType[usage.type].importSources.includes(usage.importSource)) {
  ...
  console.log(`Files with type usages: ${report.summary.filesWithUsages}`);
  console.log(`Total type usages: ${report.summary.totalUsages}`);
  console.log(`Unique types found: ${report.summary.uniqueTypes.length}`);
  console.log("");
  ...
  for (const item of report.migrationPriority.slice(0, 15)) {
    ...
    console.log(`${icon} ${item.type}`);
    console.log(`   ${item.usages} usages across ${item.files} files`);
    console.log(`   Reason: ${item.reason}`);
  }
  if (report.migrationPriority.length > 15) {
    console.log(`   ... and ${report.migrationPriority.length - 15} more types`);
  }
  console.log("");
  ...
  // Checkpoint output
  console.log("\n" + "=".repeat(60));
  console.log("✅ PHASE 0.1 CHECKPOINT");
  console.log("=".repeat(60));
  console.log(`✓ Analyzer completed successfully`);
  console.log(`✓ Found ${report.summary.uniqueTypes.length} target types`);
  console.log(`✓ High complexity types: ${report.migrationPriority.filter(p => p.complexity === "high").length}`);
  console.log(`✓ Medium complexity types: ${report.migrationPriority.filter(p => p.complexity === "medium").length}`);
  console.log(`✓ Low complexity types: ${report.migrationPriority.filter(p => p.complexity === "low").length}`);
