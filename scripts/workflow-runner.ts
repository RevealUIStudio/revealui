...
class WorkflowRunner {
	...
	async listRunningWorkflows(): Promise<void> {
		...
		for (const file of stateFiles) {
			try {
				...
				console.log(
					`  Progress: ${state.completedSteps.length}/${state.steps.length} steps`,
				);
				console.log(
					`  Current: ${state.steps[state.currentStepIndex]?.name || "N/A"}`,
				);
				if (state.reason) {
	...
	private loadTemplates(): void {
		...
		this.templates.set("automation-infrastructure", {
			...
			description:
				"Systematic approach to build real automation infrastructure",
			steps: [
				{
					id: "component-audit",
					name: "Component Audit",
					description:
						"Audit all components to identify working vs broken systems",
				...
				},
				{
					...
					description:
						"Create scripts/automation-engine.ts for real workflow execution",
				...
				},
				{
					...
					description:
						"Create scripts/review-generator.ts for generation reviews",
				...
				},
				{
					...
					description:
						"Create scripts/archive-manager.ts for project archiving",
				...
				},
				{
					...
					description:
						"Run complete validation to confirm automation infrastructure works",
					requiresApproval: false,
		...
		this.templates.set("validation-cleanup", {
			...
			steps: [
				...
				},
				{
					...
					description:
						"Update tsconfig.json files to resolve exactOptionalPropertyTypes issues",
					script: "scripts/fix-validation-issues.ts",
					requiresApproval: false,
				},
				{
					...
					description:
						"Remove incorrect biome suppressions and fix code style issues",
					requiresApproval: false,
				},
				{
					id: "validate-fixes",
...
async function main() {
	...
	if (args.length === 0) {
		...
		console.log("\nExamples:");
		console.log(
			'  workflow-runner run automation-infrastructure "Build real automation"',
		);
