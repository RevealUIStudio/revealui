...
export class FileManager {
	...
	private isImplementationReadyForReview(filePath: string): boolean {
		try {
			...
			const hasValidation =
				content.includes("## Validation") || content.includes("## Testing");
			const hasResults =
	...
	private isAnalysisReadyForPlan(filePath: string): boolean {
		try {
			...
			const hasRequirements =
				content.includes("## Requirements") || content.includes("## Must Do");
			const hasRisks =
				content.includes("## Risks") || content.includes("## Considerations");
	...
	private promoteToImplementation(planPath: string): void {
		...
		const project = this.extractProjectName(fileName);
		const newFileName = this.generateStandardName(
			project,
			"implementation",
			"in-progress",
		);
		...
		console.log(
			`🚀 Promoted plan to implementation: ${fileName} → ${newFileName}`,
		);
	}
		...
		for (const dir of dirs) {
			...
			for (const filePath of files) {
				try {
					...
					let status: "pending" | "approved" | "completed" | "archived" =
						"pending";
...
async function main() {
	...
	if (args.length === 0) {
		...
		console.log(
			"  cleanup [days] - Archive files older than N days (default: 90)",
		);
		return;
	}
		...
		case "search": {
			...
			if (searchResults.length === 0) {
				...
				for (const file of searchResults) {
					console.log(
						`  📄 ${file.name} (${file.type}, ${file.status}) - ${file.project}`,
					);
				}
			}
			break;
