...
const outputFile = './packages/services/src/core/supabase/types.ts'
...
async function generateTypes() {
  ...
  if (accessToken) {
    ...
    console.log('   Checked files:', envFiles.filter(existsSync).join(', ') || 'none found')
  }
  ...
  try {
    ...
    if (accessToken) {
      ...
      console.log('💡 Tip: Run "supabase login" or add SUPABASE_ACCESS_TOKEN to .env file')
      console.log('   Checked files:', envFiles.filter(existsSync).join(', ') || 'none found')
    }
