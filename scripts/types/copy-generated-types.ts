...
const supabaseTypesSource = join(
	rootDir,
	"packages/services/src/supabase/types.ts",
);
const supabaseTypesDest = join(
	rootDir,
	"packages/core/src/generated/types/supabase.ts",
);
const neonTypesSource = join(rootDir, "packages/db/src/types/database.ts");
const neonTypesDest = join(
	rootDir,
	"packages/core/src/generated/types/neon.ts",
);
...
export function generateNeonImports(tableMapping: TableMapping): string {
	...
	for (const subModulePath of subModules) {
		...
		if (tables.length === 1) {
			imports.push(
				`import { ${tables[0]} } from '@revealui/db/schema/${subModulePath}'`,
			);
		} else {
...
function parseImports(
	content: string,
): Array<{ tables: string[]; path: string }> {
...
export function validateTransformation(
	...
	if (importStatements.length === 0) {
		const importCount = (
			transformedContent.match(/from '@revealui\/db\/core\//g) || []
		).length;
		if (importCount === 0) {
