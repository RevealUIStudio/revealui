...
describe("copy-generated-types", () => {
	describe("discoverTableMappings", () => {
		...
		it("should handle sub-modules with hyphens correctly", () => {
			...
			const hyphenatedModules = Object.keys(mapping).filter((m) =>
				m.includes("-"),
			);
	...
	describe("generateNeonImports", () => {
		it("should generate single-line import for single table", () => {
			...
			expect(imports).toContain(
				"import { singleTable } from '@revealui/db/schema/test'",
			);
		...
		it("should include header comment", () => {
			...
			expect(imports).toContain(
				"// Import table definitions from db package sub-modules",
			);
			expect(imports).toContain(
...
// Missing agentActions, agentContexts, posts, media`;
			...
			expect(result.valid).toBe(false);
			expect(
				result.errors.some((e) => e.includes("Missing table imports")),
			).toBe(true);
		});
...
import { media } from '@revealui/db/schema/cms'`;
			...
			expect(result.valid).toBe(false);
			expect(
				result.errors.some((e) => e.includes("Duplicate table imports")),
			).toBe(true);
		});
	...
	describe("Edge cases", () => {
		...
		it("should handle invalid import syntax gracefully", () => {
			...
			expect(
				result.errors.some(
					(e) =>
						e.includes("sub-module imports") ||
						e.includes("No sub-module imports"),
				),
			).toBe(true);
		});
	});
