...
import { readdirSync, statSync, renameSync, mkdirSync, existsSync, writeFileSync, readFileSync, rmSync } from 'node:fs';
...
const REORGANIZATION_PLAN: ReorganizationPlan = {
  moves: [
    ...
    { from: 'config/turbo.json', to: 'config/build/turbo.json', description: 'Move turbo config to centralized build config' },
    { from: '.vscode-recommended.json', to: 'config/ide/vscode-recommended.json', description: 'Move VSCode config to centralized IDE config' },
    { from: 'performance.budgets.json', to: 'config/performance/budgets.json', description: 'Move performance config to centralized location' },
    { from: 'docs-lifecycle.config.json', to: 'config/docs/lifecycle.json', description: 'Move docs config to centralized location' },

    // Tool configurations
    { from: '.changeset/', to: 'config/ci/changeset/', description: 'Move changeset config to CI config' },
    { from: '.cursor/', to: 'config/ide/cursor/', description: 'Move Cursor config to IDE config' },
    { from: '.github/', to: 'config/ci/github/', description: 'Move GitHub config to CI config' },
    { from: '.husky/', to: 'config/hooks/husky/', description: 'Move Husky config to hooks config' },
    { from: '.mcp/', to: 'config/ide/mcp/', description: 'Move MCP config to IDE config' },

    // Documentation simplification
    { from: 'docs/analyses/', to: 'docs/development/analyses/', description: 'Move analyses to development docs' },
    { from: 'docs/plans/', to: 'docs/development/plans/', description: 'Move plans to development docs' },
    { from: 'docs/implementations/', to: 'docs/development/implementations/', description: 'Move implementations to development docs' },
    { from: 'docs/reviews/', to: 'docs/development/reviews/', description: 'Move reviews to development docs' },
    { from: 'docs/archives/', to: 'docs/archive/', description: 'Simplify archives location' },

    // Scripts reorganization
    { from: 'scripts/audit/', to: 'scripts/analysis/', description: 'Rename audit to analysis for clarity' },
    { from: 'scripts/consolidate-docs.ts', to: 'scripts/docs/consolidate.ts', description: 'Move docs scripts to dedicated folder' },
    { from: 'scripts/verify-claims.ts', to: 'scripts/docs/verify-claims.ts', description: 'Move docs scripts to dedicated folder' },
  ],
  creates: [
    {
...
class StructureReorganizer {
  ...
  private async updateReferences(): Promise<void> {
    ...
    const configPath = 'config/project/revealui.config.ts';
