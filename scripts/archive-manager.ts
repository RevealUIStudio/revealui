...
export class ArchiveManager {
	...
	archiveProject(projectName: string): string {
		...
		const archiveDir = join(
			this.archivesDir,
			`${timestamp}-${projectName}-complete`,
		);
	...
	private collectProjectFiles(
		...
		const sourceDirs = [
			...
			{
				path: join(process.cwd(), "docs", "implementations"),
				type: "implementation",
			},
		...
		for (const validationFile of validationFiles) {
			...
			if (existsSync(srcPath)) {
				...
				try {
					...
					console.log(
						`  ⚠️  Failed to archive ${validationFile}: ${error.message}`,
					);
				}
			}
		}
		...
		for (const automationFile of automationFiles) {
			if (automationFile.includes(projectName.replace(/[^a-zA-Z0-9]/g, ""))) {
				...
				try {
					...
					console.log(
						`  ⚠️  Failed to archive ${automationFile}: ${error.message}`,
					);
				}
			}
		}
	...
	generateIndexMarkdown(): void {
		...
		const index: ArchiveIndex = JSON.parse(
	...
	switch (command) {
		...
		case "list":
			if (existsSync(manager["indexFile"])) {
				...
				for (const [key, project] of Object.entries(index.projects)) {
					console.log(
						`  • ${project.name} (${project.archivedAt.split("T")[0]})`,
					);
				}
			} else {
