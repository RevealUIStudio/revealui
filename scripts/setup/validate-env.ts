...
async function validateEnvironment() {
	try {
		...
		if (
			process.env.REVEALUI_WHITELISTORIGINS &&
		...
		if (
			...
			process.env.REVEALUI_SECRET.length < 32
		) {
		...
		if (
			...
			if (
				...
				warnings.push(
					"⚠️  REVEALUI_PUBLIC_SERVER_URL and NEXT_PUBLIC_SERVER_URL should match",
				);
			}
		}
		...
		if (
			...
			warnings.push(
				"⚠️  POSTGRES_URL should start with postgresql:// or postgres://",
			);
		}
		...
		if (process.env.NODE_ENV === "production") {
			if (
				...
				logger.error(
					"ERROR: Production REVEALUI_PUBLIC_SERVER_URL must use HTTPS",
				);
				process.exit(1);
			}
		...
		logger.info(
			`   Required Variables: ${present.length}/${required.length} present`,
		);
		logger.info(
			`   Optional Variables: ${optionalPresent.length}/${optional.length} present`,
		);
		...
		logger.info("");
		logger.info(
			"💡 Tip: See .env.template for all available variables and documentation\n",
		);
	} catch (error) {
		logger.error(
