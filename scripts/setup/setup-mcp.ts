...
async function setupMCP() {
	try {
		...
		logger.info(
			"  Official Stripe MCP server with 20+ payment & billing tools\n",
		);
		...
		logger.info(
			"1. Configure your AI client (Claude, Cursor, etc.) to connect to MCP servers",
		);
		...
		if (!vercelToken || !stripeKey) {
			logger.warning(
				"⚠️  Please set up your API keys in .env before running MCP servers\n",
			);
			process.exit(1);
		} else {
			logger.success(
				'🎉 Ready to launch! Run "pnpm mcp:all" to start all MCP servers\n',
			);
		}
	} catch (error) {
		logger.error(
