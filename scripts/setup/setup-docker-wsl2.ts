...
async function addDockerGPGKey() {
	...
	await execCommand("sudo", ["mkdir", "-p", "/etc/apt/keyrings"], {
		silent: true,
	});
...
async function addDockerRepository() {
	...
	const archResult = await execCommand("dpkg", ["--print-architecture"], {
		silent: true,
	});
	const arch = archResult.message.trim();

	const codenameResult = await execCommand("lsb_release", ["-cs"], {
		silent: true,
	});
...
async function _main() {
	...
	logger.warning(
		"IMPORTANT: You need to log out and log back in (or restart WSL)",
	);
