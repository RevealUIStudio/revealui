...
async function syncEnvVars() {
	try {
		...
		logger.info(
			`Found ${foundVars.length}/${REQUIRED_VARS.length} required variables in root .env`,
		);

		if (missingVars.length > 0) {
			logger.warning(
				`Missing variables in root .env: ${missingVars.join(", ")}`,
			);
		}
		...
		logger.success(
			`✅ Synced ${updatedVars.size} environment variables to .env.development.local`,
		);
