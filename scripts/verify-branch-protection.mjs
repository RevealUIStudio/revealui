...
try {
  ...
  const securityChecks = [
    'snyk-security', 'secret-scanning', 'dependency-review', 'codeql-analysis'
  ];

  // Check for missing critical checks
  const missingCritical = criticalChecks.filter(check =>
    !requiredChecks.includes(check)
  );

  const missingSecurity = securityChecks.filter(check =>
    !requiredChecks.includes(check)
  );

  if (missingCritical.length > 0) {
    console.error(`❌ Missing critical CI checks: ${missingCritical.join(', ')}`);
  ...
  if (!reviews) {
    ...
    console.log(`   • Required approving reviews: ${reviews.required_approving_review_count || 0}`);
    console.log(`   • Dismiss stale reviews: ${reviews.dismiss_stale_reviews ? 'Yes' : 'No'}`);
    console.log(`   • Require code owner reviews: ${reviews.require_code_owner_reviews ? 'Yes' : 'No'}`);
  ...
  // Overall assessment
  const issues = missingCritical.length + missingSecurity.length +
