...
try {
  ...
  const coreDeps = Object.keys(corePackage.dependencies || {}).filter((dep) =>
  ...
  const dbDeps = Object.keys(dbPackage.dependencies || {}).filter((dep) =>
    dep.startsWith('@revealui/'),
  )
  ...
  const contractsDeps = Object.keys(contractsPackage.dependencies || {}).filter((dep) =>
    dep.startsWith('@revealui/'),
  )
...
try {
  ...
  if (
    typeAdapterContent.includes('@revealui/contracts/generated') &&
