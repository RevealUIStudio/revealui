...
async function migrateVectorData(
	...
	try {
		...
		for (let i = 0; i < allMemories.length; i += batchSize) {
			...
			console.log(
				`Processing batch ${batchNum}/${totalBatches} (${batch.length} memories)...`,
			);
					...
					const embeddingVector = memory.embedding
						? Array.isArray(memory.embedding)
							...
							: typeof memory.embedding === "string"
								? JSON.parse(
										memory.embedding.replace(/^\[/, "[").replace(/\]$/, "]"),
									)
								: []
						: null;
		...
		if (!dryRun && stats.errors === 0) {
			...
			const restResult = await restDb.execute(
				sql`SELECT COUNT(*) as count FROM agent_memories`,
			);
			const restRows = Array.isArray(restResult)
				? restResult
				: (restResult as any).rows || [];
