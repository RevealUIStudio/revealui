...
async function setupFreshDatabase() {
	try {
		...
		if (!connectionString) {
			...
			logger.error(
				"Set one of: DATABASE_URL, POSTGRES_URL, or SUPABASE_DATABASE_URI",
			);
			process.exit(1);
		}
		...
		try {
			...
			try {
				...
				if (error.message?.includes("permission denied")) {
					logger.warning(
						"Could not enable vector extension (may need superuser)",
					);
					logger.info(
      ...
      `);
			...
			const passwordHashSQL = fs.readFileSync(
				passwordHashMigrationPath,
				"utf-8",
			);
			...
			for (const stmt of statements) {
				try {
					...
					if (
						...
						logger.info(
							`   Skipping (already exists): ${stmt.substring(0, 50)}...`,
						);
					} else {
						throw error;
					}
			...
			if (fs.existsSync(emailUniqueMigrationPath)) {
				const emailUniqueSQL = fs.readFileSync(
					emailUniqueMigrationPath,
					"utf-8",
				);
