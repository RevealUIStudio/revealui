...
function classifyTask(description: string): TaskAnalysis {
  ...
  if (lowerDesc.includes('fix') || lowerDesc.includes('bug') || lowerDesc.includes('error')) {
    type = 'bug-fix'
  } else if (lowerDesc.includes('refactor') || lowerDesc.includes('architecture')) {
  ...
  if (lowerDesc.includes('complex') || lowerDesc.includes('major') || lowerDesc.length > 200) {
    complexity = 'complex-effort'
  }

  if (lowerDesc.includes('critical') || lowerDesc.includes('urgent') || lowerDesc.includes('block')) {
    priority = 'critical'
  }
...
function extractRequirements(description: string): string[] {
  ...
  return requirements.length > 0 ? requirements : ['Analyze and implement requirements from description']
}
...
async function main() {
  ...
  const filename = `smart-dev-${Date.now()}-${description.slice(0, 50).replace(/[^a-zA-Z0-9]/g, '-').toLowerCase()}.md`
