...
if (!POSTGRES_URL) {
	...
	console.warn(
		"📖 See packages/memory/TESTING.md for testing limitations and validation strategy",
	);
}
...
describe.skipIf(!POSTGRES_URL)("Automated CRDT Validation", () => {
	...
	describe("Node ID Persistence", () => {
		it("should persist node IDs across requests", async () => {
			const nodeId1 = await nodeIdService.getNodeId(
				"session",
				"test-session-1",
			);
			const nodeId2 = await nodeIdService.getNodeId(
				"session",
				"test-session-1",
			);
	...
	describe("Embedding Metadata Storage", () => {
		it("should store full embedding metadata", async () => {
			...
			expect((dbMemory?.embeddingMetadata as any)?.model).toBe(
	...
	describe("Performance Benchmarks", () => {
		...
		it("should handle concurrent lookups efficiently", async () => {
			...
			for (let i = 0; i < iterations; i++) {
				promises.push(
					nodeIdService.getNodeId("session", `concurrent-perf-${i}`),
				);
