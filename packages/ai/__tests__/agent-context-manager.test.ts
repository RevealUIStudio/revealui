...
import { AgentContextManager } from "../src/memory/agent/context-manager";
import { ValidationError } from "../src/memory/errors";
import { WorkingMemory } from "../src/memory/memory/working-memory";
import type { CRDTPersistence } from "../src/memory/persistence/crdt-persistence";
...
describe("AgentContextManager", () => {
	...
	describe("Input Validation", () => {
		it("should reject dangerous keys in setContext", () => {
			expect(() => manager.setContext("__proto__", { polluted: true })).toThrow(
				ValidationError,
			);
			expect(() => manager.setContext("constructor", {})).toThrow(
				ValidationError,
			);
			expect(() => manager.setContext("prototype", {})).toThrow(
				ValidationError,
			);
		});

		it("should reject undefined values in setContext", () => {
			expect(() => manager.setContext("key", undefined)).toThrow(
				ValidationError,
			);
		});

		it("should reject functions in setContext", () => {
			expect(() => manager.setContext("key", () => {})).toThrow(
				ValidationError,
			);
		});
		...
		it("should reject dangerous keys in updateContext", () => {
			...
			expect(() =>
				manager.updateContext({ constructor: { polluted: true } }),
		...
		it("should reject circular references in updateContext", () => {
			...
			expect(() => manager.updateContext({ circular })).toThrow(
				ValidationError,
			);
		});
		...
		it("should reject dangerous keys in mergeContext", async () => {
			...
			await expect(
				manager.mergeContext({ constructor: { polluted: true } }),
			...
			await expect(manager.mergeContext(protoContext)).rejects.toThrow(
				ValidationError,
			);
		});
		...
		it("should reject circular references in mergeContext", async () => {
			const circular: any = { a: 1 };
			circular.self = circular;
			await expect(manager.mergeContext({ circular })).rejects.toThrow(
				ValidationError,
			);
		});
