...
import { LWWRegister, ORSet, PNCounter, VectorClock } from '../src/memory/crdt/index'
...
describe('LWWRegister', () => {
  ...
  describe('Immutability Protection', () => {
    it('should prevent mutation of values returned from get()', () => {
      const reg = new LWWRegister('node1', { foo: 'bar', nested: { value: 42 } })
    ...
    it('should prevent mutation of nested objects in merged values', () => {
      ...
      const merged = reg1.merge(reg2)
      const mergedValue = merged.get() as { nested: { deep: { value: string } } }
