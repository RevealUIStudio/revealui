...
export class TicketAgentDispatcher {
  ...
  async dispatch(ticket: TicketInput): Promise<AgentResult> {
    const { apiClient, ticketClient, collections, globals, memory: sharedMemory } = this.config

    // Build tool set: CMS tools + ticket mutation tools
    ...
    const tools = [...cmsTools, ...ticketTools]
    ...
    const agent: Agent = {
      ...
      execute(task: Task): Promise<AgentResult> {
        // The runtime calls this interface; we delegate back through the orchestrator
        return Promise.resolve({ success: false, error: 'Use orchestrator.delegateTask() instead' })
      },
      getContext() {
        return {
    ...
    const taskDescription = descriptionText ? `${ticket.title}\n\n${descriptionText}` : ticket.title
