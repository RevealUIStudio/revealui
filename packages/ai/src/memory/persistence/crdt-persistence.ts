...
export class CRDTPersistence {
	...
	async loadCRDTState(
		...
		LWWRegisterData<unknown> | ORSetData<unknown> | PNCounterData | null
	> {
		...
		return crdtData[type] as
			| LWWRegisterData<unknown>
			| ORSetData<unknown>
			| PNCounterData;
	}
	...
	async getOperationsSince(
		crdtId: string,
		since: number,
	): Promise<CRDTOperationPayload[]> {
		const operations = await this.db
			.select()
			.from(crdtOperations)
			.where(
				and(
					eq(crdtOperations.crdtId, crdtId),
					gte(crdtOperations.timestamp, since),
				),
			)
	...
	async loadCompositeState(
		...
		Map<string, LWWRegisterData<unknown> | ORSetData<unknown> | PNCounterData>
	> {
		const context = await findAgentContextById(this.db, crdtId);
		...
		for (const [key, data] of Object.entries(crdtData)) {
			if (data && typeof data === "object") {
				result.set(
					key,
					data as LWWRegisterData<unknown> | ORSetData<unknown> | PNCounterData,
				);
			}
		}
