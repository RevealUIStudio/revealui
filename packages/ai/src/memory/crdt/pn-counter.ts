 ...
 * console.log(merged.value()) // 6 (5 + 3 - 2)
 * ```
 */
...
export class PNCounter {
	...
	increment(delta: number = 1): void {
		if (delta < 0) {
			throw new Error(
				"Increment delta must be non-negative. Use decrement for negative values.",
			);
		}
	...
	decrement(delta: number = 1): void {
		if (delta < 0) {
			throw new Error(
				"Decrement delta must be non-negative. Use increment for negative values.",
			);
		}
	...
	get nodes(): string[] {
		return Array.from(
			new Set([...this.increments.keys(), ...this.decrements.keys()]),
		);
	}
