...
export class WorkingMemory {
	...
	merge(other: WorkingMemory): WorkingMemory {
		const merged = new WorkingMemory(
			this.sessionId,
			this.nodeId,
			this.persistence,
		);
	...
	async load(): Promise<void> {
		if (!this.persistence) {
			throw new Error(
				"Persistence not configured. Pass persistence to constructor.",
			);
		}
		...
		if (agentsData && "added" in agentsData) {
			const agentsSet = ORSet.fromData<AgentDefinition>(
				agentsData as ORSetData<AgentDefinition>,
			);
