 ...
 *
 * Note: This is a simplified deep clone for our specific use case (UserPreferences).
 * For more complex needs, consider a library like lodash.cloneDeep.
 */
...
export function deepClone<T>(obj: T, visited = new WeakMap<object, unknown>()): T {
  ...
  if (ArrayBuffer.isView(obj)) {
    const typedArray = obj as unknown as { buffer: ArrayBuffer; byteOffset: number; length: number }
  ...
  // For UserPreferences, functions shouldn't exist, but we handle gracefully
  ...
  // For UserPreferences, this shouldn't happen, but we handle it gracefully
  return obj
}
