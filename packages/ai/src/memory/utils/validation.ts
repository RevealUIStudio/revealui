...
export function validateContextKey(key: string): void {
	if (typeof key !== "string") {
		throw new ValidationError(
			`Context key must be a string, got: ${typeof key}`,
		);
	}
	...
	if (key.length > 256) {
		throw new ValidationError(
			`Context key too long: ${key.length} characters (max 256)`,
		);
	}
...
export function validateContextValue(value: unknown, key: string): void {
	if (value === undefined) {
		throw new ValidationError(
			`Context values cannot be undefined. Key: ${key}`,
		);
	}

	if (typeof value === "function") {
		throw new ValidationError(
			`Context values cannot be functions. Key: ${key}`,
		);
	}
...
export function validateObjectDepth(
	...
	if (Array.isArray(obj)) {
		if (maxDepth <= 0) {
			throw new ValidationError(
				`Object depth exceeds maximum of ${MAX_OBJECT_DEPTH}`,
			);
		}
		let maxChildDepth = 0;
...
export function estimateObjectSize(obj: unknown): number {
	...
	if (type === "string") {
		return (obj as string).length * 2 + 8; // UTF-16 encoding
	}
...
export function validateContextSize(context: Record<string, unknown>): void {
	...
	if (keyCount > MAX_CONTEXT_KEYS) {
		throw new ValidationError(
			`Context has too many keys: ${keyCount} (max ${MAX_CONTEXT_KEYS})`,
		);
	}
