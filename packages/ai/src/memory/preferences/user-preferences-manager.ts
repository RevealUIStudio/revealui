...
export class UserPreferencesManager {
	...
	private db: Database;
	private logger = createLogger("[UserPreferences]");
	...
	merge(other: UserPreferencesManager): UserPreferencesManager {
		const merged = new UserPreferencesManager(
			this.userId,
			this.nodeId,
			this.db,
		);
		...
		try {
			...
			if (
				...
				if (prefsRecord._crdt && typeof prefsRecord._crdt === "object") {
					...
					const lwwData = crdtData.lww_register as
						| LWWRegisterData<UserPreferences>
						| undefined;
	...
	clone(): UserPreferencesManager {
		return UserPreferencesManager.fromData(
			this.toData(),
			this.userId,
			this.nodeId,
			this.db,
