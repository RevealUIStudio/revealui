export async function routeModel(prompt: string) {
  ...
  if (!prompt || prompt.length < 600) {
    const mod = await import('../../packages/ai/src/inference/vultr')
    return (mod as any).default || mod
  }

  if (prompt.includes('```')) {
    const mod = await import('../../packages/ai/src/inference/docker')
    return (mod as any).default || mod
  }

  const mod = await import('../../packages/ai/src/inference/huggingface')
  return (mod as any).default || mod
}
