...
// Usage:
// VULTR_API_KEY=your_key VULTR_MODEL=your-model-id node packages/ai/scripts/test-vultr.mjs
...
async function chat(prompt) {
  ...
  const message = choice && choice.message ? choice.message : choice && choice.text ? { content: choice.text } : undefined
  if (message) {
    console.log('\nAssistant output:')
...
// Node 18+ supports top-level await in ESM; call main
main()
