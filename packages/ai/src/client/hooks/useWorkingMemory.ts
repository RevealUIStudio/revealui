...
export interface UseWorkingMemoryReturn {
	...
	updateSessionState: (
		state: Partial<UseWorkingMemoryReturn["sessionState"]>,
	) => Promise<void>;
	activeAgents: Array<{
		id: string;
		name: string;
...
export function useWorkingMemory(
	...
	const [sessionState, setSessionStateState] = useState<
		UseWorkingMemoryReturn["sessionState"]
	>({
		status: "active",
	});
	const [activeAgents, setActiveAgents] = useState<
		UseWorkingMemoryReturn["activeAgents"]
	>([]);
	...
	useEffect(() => {
		...
		async function load() {
			try {
				...
				if (!response.ok) {
					throw new Error(
						`Failed to load working memory: ${response.statusText}`,
					);
				}
	...
	const sync = useCallback(async () => {
		try {
			...
			if (!response.ok) {
				throw new Error(
					`Failed to sync working memory: ${response.statusText}`,
				);
			}
	...
	const updateSessionState = useCallback(
		async (state: Partial<UseWorkingMemoryReturn["sessionState"]>) => {
			try {
				...
				if (!response.ok) {
					throw new Error(
						`Failed to update session state: ${response.statusText}`,
					);
				}
