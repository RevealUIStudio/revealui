export default {
  ...
  async generate({ prompt }: { prompt?: string }) {
    ...
    const url = process.env.HF_MODEL_URL || 'https://router.huggingface.co/models/mistralai/Mistral-7B-Instruct-v0.3'
    ...
    if (contentType.includes('application/json')) {
      ...
      text = (json as any)?.generated_text || (Array.isArray(json) ? (json as any)[0]?.generated_text : null) || JSON.stringify(json)
    } else {
