...
[pnpm workspaces](https://docs.npmjs.com/cli/v10/using-npm/workspaces) to
configure the monorepo. This mostly means that all packages share a
top-level `pnpm-lock.yaml` and `pnpm run {command} -w {package}` is often
...
```
pnpm create -w packages/lexical-eslint-plugin
```
...
- Create a local `${pnpm_package_version}__release` branch
