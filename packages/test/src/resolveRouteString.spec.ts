...
import { resolveRouteString } from "reveal/core/router/resolveRouteString"
...
describe("resolveRouteString", () => {
  ...
  it("parameterized routes", () => {
    ...
    expect(r("/@p1/@p2", "/a/b")).toEqual({
      routeParams: { p1: "a", p2: "b" },
    })
    expect(r("/@p1/@p2", "/a/b/")).toEqual({
      routeParams: { p1: "a", p2: "b" },
    })
  ...
  it("glob - middle", () => {
    ...
    expect(r("/a/*/e", "/a/b/c/d/e")).toEqual({
      routeParams: { "*": "b/c/d" },
    })
    expect(r("/a*e", "/a/b/c/d/e")).toEqual({
      routeParams: { "*": "/b/c/d/" },
  ...
  it("glob - multiple", () => {
    expect(r("/a/*/c/*/e", "/a/b/c/d/e")).toEqual({
      routeParams: { "*1": "b", "*2": "d" },
    })
    expect(r("/a/*/c/*", "/a/b/c/d/e")).toEqual({
      routeParams: { "*1": "b", "*2": "d/e" },
    })
    expect(r("/a/*/c/*", "/a/b/c/d/e/")).toEqual({
      routeParams: { "*1": "b", "*2": "d/e/" },
    })
    expect(r("*a*", "/a/b/c/d/e")).toEqual({
      routeParams: { "*1": "/", "*2": "/b/c/d/e" },
    })
    expect(r("*a*e", "/a/b/c/d/e")).toEqual({
      routeParams: { "*1": "/", "*2": "/b/c/d/" },
    })
    expect(r("*a*c", "/a/b/c/d/e")).toEqual(null)
    expect(r("*a*c*", "/a/b/c/d/e")).toEqual({
      routeParams: { "*1": "/", "*2": "/b/", "*3": "/d/e" },
    })
  })
  it("glob - edge cases", () => {
    expect(r("/a*b*c", "/abc")).toEqual({
      routeParams: { "*1": "", "*2": "" },
    })
    expect(r("/a*b*c*", "/abc")).toEqual({
      routeParams: { "*1": "", "*2": "", "*3": "" },
  ...
  it("glob - complex non-linear matching", () => {
    ...
    expect(r("/a*a*b*", "/aaaaaba")).toEqual({
      routeParams: { "*1": "aaa", "*2": "", "*3": "a" },
    })
    expect(r("/a*b*0*", "/aabcbc0")).toEqual({
      routeParams: { "*1": "abc", "*2": "c", "*3": "" },
    })
    expect(r("/a*b*0*", "/aab0cbc")).toEqual({
      routeParams: { "*1": "a", "*2": "", "*3": "cbc" },
    })
  })
  it("glob - BurdaForward", () => {
    // Use case 1
    expect(r("/some/*.html", "/some/a.html")).toEqual({
      routeParams: { "*": "a" },
    })
    expect(r("/some/*.html", "/some/a/b/c.html")).toEqual({
      routeParams: { "*": "a/b/c" },
    })
    // Use case 2
    expect(r("/some/route*.html", "/some/route.html")).toEqual({
      routeParams: { "*": "" },
    })
    expect(r("/some/route*.html", "/some/route-a.html")).toEqual({
      routeParams: { "*": "-a" },
    })
    expect(r("/some/route*.html", "/some/routea/b/c.html")).toEqual({
      routeParams: { "*": "a/b/c" },
    })
  })
  ...
  it("URL with trailing slash", () => {
    ...
    expect(r("/news/press-releases*", "/news/press-releases")).toEqual({
      routeParams: { "*": "" },
    })
  })
  ...
  it("invalid route string", () => {
    expectErr(
      () => r("", "/a/b/c"),
      `[reveal][Wrong Usage] Invalid Route String '' (empty string): set it to / instead`
    )
    expectErr(
      () => r("a", "/a/b/c"),
      `[reveal][Wrong Usage] Invalid Route String a: it should start with / or *`
    )
    expectErr(
      () => r("/a**b", "/a/b/c"),
      `[reveal][Wrong Usage] Invalid Route String /a**b: set it to /a*b instead`
    )
  })
})
