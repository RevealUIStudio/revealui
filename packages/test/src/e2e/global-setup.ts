	...
	try {
		...
		while (retries > 0) {
			try {
				...
				if (
					...
					console.log(
						`[E2E Setup] ✅ Server is ready (HTTP ${response.status()})`,
					);
					await browser.close();
					return;
				}
			...
			if (retries > 0) {
				console.log(
					`[E2E Setup] Server not ready, retrying... (${retries} attempts left)`,
				);
		...
		throw new Error(
			`E2E test setup failed - server not ready at ${baseURL}: ${errorMessage}. ` +
				"Start the server with `pnpm dev` in apps/cms or apps/web. " +
