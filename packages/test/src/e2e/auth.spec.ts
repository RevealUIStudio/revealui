...
test.describe("Authentication E2E Tests", () => {
	...
	test.describe("Sign In Flow", () => {
		...
		test("should successfully sign in with correct credentials", async ({
			...
			expect(
				cookies?.some((cookie) => cookie.includes("revealui-session")),
			).toBe(true);
		});
		...
		test("should fail to sign in with non-existent email", async ({
			request,
		}) => {
			const response = await request.post(`${API_BASE}/sign-in`, {
				data: {
	...
	test.describe("Protected Routes", () => {
		...
		test("should access protected shape proxy route with valid session", async ({
			...
			const response = await request.get(
				`${BASE_URL}/api/shapes/agent-contexts`,
				{
					headers: {
						Cookie: `revealui-session=${sessionToken}`,
					},
	...
	test.describe("Rate Limiting", () => {
		test("should rate limit after multiple failed sign-in attempts", async ({
			request,
		}) => {
			// Create user first
			await request.post(`${API_BASE}/sign-up`, {
