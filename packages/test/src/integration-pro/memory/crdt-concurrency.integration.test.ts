/**
 * Pro CRDT Concurrency Integration Tests
 *
...
const crdtModule = await import('@revealui/ai/memory/crdt').catch(() => null);
const describeIfPro = crdtModule ? describe : describe.skip;

describeIfPro('CRDT Concurrency Integration Tests', () => {
  const { LWWRegister, ORSet, PNCounter, VectorClock } = crdtModule!;
  const createStringRegister = (nodeId: string, initialValue: string, timestamp?: number) =>
    new LWWRegister<string>(nodeId, initialValue, timestamp);
  const createCounter = (nodeId: string) => new PNCounter(nodeId);
  ...
  describe('LWWRegister Concurrent Writes', () => {
    ...
    it('should handle rapid concurrent merges without data loss', () => {
      ...
      const registers: Array<ReturnType<typeof createStringRegister>> = [];
      for (let i = 0; i < 10; i++) {
        const reg = createStringRegister(`node-${i}`, 'initial', baseTimestamp);
  ...
  describe('PNCounter Distributed Counting', () => {
    it('should correctly sum increments from multiple nodes', () => {
      // Create 5 nodes
      const counters: Array<ReturnType<typeof createCounter>> = [];
      for (let i = 0; i < 5; i++) {
        const counter = createCounter(`node-${i}`);
