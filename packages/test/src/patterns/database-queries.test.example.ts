...
describe("Database Query Testing Patterns", () => {
	...
	describe("Query Execution", () => {
		...
		it("should execute INSERT queries", async () => {
			...
			const result = await db.query("SELECT * FROM test_users WHERE id = ?", [
				"1",
			]);
		...
		it("should execute UPDATE queries", async () => {
			...
			await db.query("UPDATE test_users SET name = ? WHERE id = ?", [
				"Updated Name",
				"1",
			]);
	...
	describe("Transactions", () => {
		it("should commit transaction successfully", async () => {
			await db.transaction(async () => {
				await db.query(
					"INSERT INTO test_users (id, email, name) VALUES (?, ?, ?)",
					["1", "test1@example.com", "User 1"],
				);
				await db.query(
					"INSERT INTO test_users (id, email, name) VALUES (?, ?, ?)",
					["2", "test2@example.com", "User 2"],
				);
			});
