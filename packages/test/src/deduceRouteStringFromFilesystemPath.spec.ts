import { deduceRouteStringFromFilesystemPath } from "reveal/core/router/deduceRouteStringFromFilesystemPath";
...
describe("deduceRouteStringFromFilesystemPath", () => {
  ...
  it("filesystem roots", () => {
    ...
    expect(deduceRouteStringFromFilesystemPath(pS[1]!, [])).toBe(
      "/admin/login",
    );
    expect(deduceRouteStringFromFilesystemPath(pS[3]!, [])).toBe("/about");
    const fsBase = { filesystemRoot: "/pages/admin", urlRoot: "/_admin" };
    expect(deduceRouteStringFromFilesystemPath(pS[0]!, [fsBase])).toBe(
      "/_admin",
    );
    expect(deduceRouteStringFromFilesystemPath(pS[1]!, [fsBase])).toBe(
      "/_admin/login",
    );
    expect(deduceRouteStringFromFilesystemPath(pS[3]!, [fsBase])).toBe(
      "/about",
    );
  });
  ...
  it("domain-driven file structure", () => {
    ...
    expect(deduceRouteStringFromFilesystemPath(pS[5]!, [fsBase])).toBe(
      "/about",
    );
  });
