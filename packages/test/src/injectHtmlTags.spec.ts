import {
  injectAtOpeningTag,
  injectAtClosingTag,
} from "reveal/core/assets/injectHtmlTags";
...
describe("injectHtmlTags", () => {
  it("injectAtOpeningTag()", () => {
    ...
    expect(
      injectAtOpeningTag("head", htmlHead1, `${tag1}${tag1}`),
    ).toMatchInlineSnapshot(
      `"<html><head><link href="/foo.ttf"><link href="/foo.ttf"></head></html>"`,
    );
    expect(
      injectAtOpeningTag("head", htmlHead2, `${tag1}${tag1}`),
    ).toMatchInlineSnapshot(
      `
      "<html>
        <head>
    ...
    expect(
      injectAtOpeningTag("head", htmlHead3, `${tag2}${tag2}${tag1}`),
    ).toMatchInlineSnapshot(
      `
      "<html>
        <head>
  ...
  it("injectAtClosingTag()", () => {
    ...
    expect(
      injectAtClosingTag("body", htmlBody1, `${tag1}${tag1}`),
    ).toMatchInlineSnapshot(
      `"<html><body><link href="/foo.ttf"><link href="/foo.ttf"></body></html>"`,
    );
    expect(
      injectAtClosingTag("body", htmlBody2, `${tag1}${tag1}`),
    ).toMatchInlineSnapshot(
      `
      "<html>
        <body>
    ...
    expect(
      injectAtClosingTag("body", htmlBody3, `${tag1}${tag2}${tag1}${tag2}`),
    ).toMatchInlineSnapshot(
      `
      "<html>
        <body>
