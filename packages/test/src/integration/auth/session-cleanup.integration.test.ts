...
describe('Session Cleanup Integration Tests', () => {
  ...
  describe('Bulk Session Deletion', () => {
    it('should delete all sessions for a user', async () => {
      ...
      const beforeSessions = await db.select().from(sessions).where(eq(sessions.userId, testUserId))
  ...
  describe('Session Metadata', () => {
    it('should store userAgent and ipAddress', async () => {
      ...
      expect(session.userAgent).toBe('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36')
