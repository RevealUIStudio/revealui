...
describe("Database Queries Integration", () => {
	...
	let revealui: ReturnType<typeof getTestRevealUI> extends Promise<infer T>
		? T
		: never;
	...
	describe("Query Execution", () => {
      ...
      `);
			...
			await db.query("INSERT INTO test_table (id, name) VALUES (?, ?)", [
				"1",
				"Test",
			]);

			const result = await db.query("SELECT * FROM test_table WHERE id = ?", [
				"1",
			]);
			expect(result.rows).toHaveLength(1);
      ...
      `);
			...
			await db.query("UPDATE test_table SET name = ? WHERE id = ?", [
				"Updated",
				"1",
			]);
	...
	describe("Query Error Handling", () => {
      ...
      `);
			...
			await expect(
				db.query("SELECT * FROM test_table WHERE id = ?", []),
			).rejects.toThrow();
	...
	describe("SQL Syntax Fixes - WHERE Clause", () => {
		...
		it("should handle WHERE clause with nested AND/OR", async () => {
			...
			const result = await revealui.find({
				...
				where: {
					or: [
						{ email: { equals: testEmail1 } },
						{ email: { equals: testEmail2 } },
					],
				},
			});
