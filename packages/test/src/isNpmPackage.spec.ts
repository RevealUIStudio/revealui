...
import {
  ...
  isValidPathAlias,
  parse,
} from "reveal/core/gaurds/validators/isNpmPackage";

describe("parse()", () => {
  it("basics", () => {
    expect(parse("some-pkg")).toStrictEqual({
      pkgName: "some-pkg",
      importPath: null,
    });
    expect(parse("@scope/name")).toStrictEqual({
      pkgName: "@scope/name",
      importPath: null,
    });
    expect(parse("@scope/name/path")).toStrictEqual({
      pkgName: "@scope/name",
      importPath: "path",
    });
