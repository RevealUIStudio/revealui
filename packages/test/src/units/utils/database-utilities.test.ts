...
describe("SQLite Database Utilities", () => {
	...
	describe("query execution", () => {
      ...
      `);
			...
			const result = await adapter.query("SELECT * FROM test WHERE id = ?", [
				"1",
			]);
			expect(result.rows).toHaveLength(1);
      ...
      `);
			...
			await adapter.query("INSERT INTO test (id, name) VALUES ($1, $2)", [
				"1",
				"Test",
			]);
      ...
      `);
			...
			await adapter.query("UPDATE test SET name = ? WHERE id = ?", [
				"Updated",
				"1",
			]);
      ...
      `);
		...
		it("should throw error when querying without connection", async () => {
			...
			await expect(uninitializedAdapter.query("SELECT 1")).rejects.toThrow(
	...
	describe("table creation", () => {
		...
		it("should create table with text fields", async () => {
			...
			const descriptionColumn = result.rows.find(
				(row: any) => row.name === "description",
			);
			...
			const createdAtColumn = result.rows.find(
		...
		it("should create table with checkbox fields", async () => {
			...
			const publishedColumn = result.rows.find(
				(row: any) => row.name === "published",
			);
		...
		it("should create table with unique fields", async () => {
			...
			const indexDetails = await adapter.query(
				`PRAGMA index_info("${indexInfo.rows[0].name}")`,
			);
		...
		it("should create table with standard columns", async () => {
			...
			const updatedAtColumn = result.rows.find(
	...
	describe("transactions", () => {
      ...
      `);
		...
		it("should rollback transaction on error with sync operations", async () => {
			...
			const rollbackRecord = await adapter.query(
			...
			const beforeRecord = await adapter.query(
				"SELECT * FROM test WHERE id = ?",
				["before-transaction"],
			);
