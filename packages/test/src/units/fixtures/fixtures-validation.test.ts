...
describe("Fixture Validation (Testing Infrastructure)", () => {
	describe("User Fixtures", () => {
		...
		it("should have default test users with valid data", () => {
			// Test admin user
			const adminEmailResult = emailSchema.safeParse(
				defaultTestUsers.admin.email,
			);
			expect(adminEmailResult.success).toBe(true);

			const adminPasswordResult = passwordSchema.safeParse(
				defaultTestUsers.admin.password,
			);
			expect(adminPasswordResult.success).toBe(true);

			// Test regular user
			const regularEmailResult = emailSchema.safeParse(
				defaultTestUsers.regular.email,
			);
			expect(regularEmailResult.success).toBe(true);

			// Test guest user
			const guestEmailResult = emailSchema.safeParse(
				defaultTestUsers.guest.email,
			);
	...
	describe("Payment Fixtures", () => {
		...
		it("should have default test payments with valid structure", () => {
			...
			expect(defaultTestPayments.medium.amount).toBeGreaterThan(
				defaultTestPayments.small.amount,
			);
