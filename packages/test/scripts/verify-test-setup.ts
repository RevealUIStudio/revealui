...
async function verifyConnectionStringFormat() {
	...
	if (dbUrl) {
		const isValid =
			/^postgresql:\/\//.test(dbUrl) || /^postgres:\/\//.test(dbUrl);
		addResult({
	...
	if (postgresUrl) {
		const isValid =
			/^postgresql:\/\//.test(postgresUrl) ||
			/^postgres:\/\//.test(postgresUrl);
		addResult({
...
async function verifySupabaseSchema() {
	...
	try {
		...
		const existingColumns = columns.map(
			(c: any) => c.column_name || c.column_name,
		);
	...
	try {
		...
		if (!extensionExists) {
			addResult({
				...
				details:
					"Run: CREATE EXTENSION IF NOT EXISTS vector; in your Supabase database",
			});
			return;
		}
...
async function verifyIndexes() {
	...
	try {
		...
		const indexes = Array.isArray(indexCheck)
			? indexCheck
			: (indexCheck as any).rows || [];

		const hasEmbeddingIndex = indexes.some(
			(idx: any) =>
				idx.indexname?.includes("embedding") ||
				idx.indexdef?.includes("embedding"),
		);
		...
		if (!hasEmbeddingIndex) {
			addResult({
				...
				details:
					"Performance may be degraded. Run migration to create HNSW index.",
			});
		} else {
			addResult({
				name: "HNSW Index",
