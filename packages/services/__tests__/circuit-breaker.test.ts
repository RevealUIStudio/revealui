...
describe("Circuit Breaker State Transitions", () => {
	...
	it("should open circuit after 5 consecutive failures", async () => {
		...
		expect(mockStripe.customers.create).toHaveBeenCalledTimes(
			5 * retryConfig.maxAttempts,
		);
	...
	it("should transition to half-open after reset timeout", async () => {
		...
		const stateAfterCall = testUtils.getCircuitBreakerState(
			"paymentIntents.create",
		);
		expect(stateAfterCall.state).toBe("half-open");

		// After one success in half-open, successCount should be 1
		const stateAfter = testUtils.getCircuitBreakerState(
			"paymentIntents.create",
		);
