...
describe.skipIf(!hasStripeKey)("Stripe Integration Tests", () => {
	...
	beforeAll(() => {
		...
		if (!process.env.STRIPE_SECRET_KEY?.startsWith("sk_test_")) {
			throw new Error(
				"STRIPE_SECRET_KEY must be set to a test key for integration tests",
			);
		}
	});
	...
	it("should create payment intent with real product/price", async () => {
		...
		try {
			...
			// Create payment intent using protectedStripe
	...
	it("should handle breaker behavior under repeated invalid requests", async () => {
		...
		// Note: This test may not always trigger the breaker due to retry logic,
		// but it verifies the breaker mechanism exists
