...
describe("services - Functionality Tests", () => {
	...
	describe("createPaymentIntent", () => {
		...
		describe("Success Path Tests", () => {
			it("should successfully create payment intent with existing customer", async () => {
				...
				if ("send" in result && result.send) {
					expect(result.send).toHaveProperty(
						"client_secret",
						"pi_test_123_secret_test123",
					);
				}
		...
		describe("Edge Cases", () => {
				...
				const createMockReq = (userId: string): RevealRequest => ({
					...
					revealui: {
						findByID: vi.fn().mockResolvedValue({
							...
							cart: {
								items: [
									{ product: { stripeProductID: "prod_test" }, quantity: 1 },
								],
							},
						}),
						update: vi.fn().mockResolvedValue({}),
			...
			it("should verify circuit breaker isolation under concurrency", async () => {
				...
				const { testUtils } = await import(
					"../src/core/stripe/stripeClient.test-utils"
				);
				...
				const customersState =
					testUtils.getCircuitBreakerState("customers.create");
				expect(customersState.state).toBe("open");

				const paymentIntentsState = testUtils.getCircuitBreakerState(
		...
		describe("Stripe Integration Tests (Optional)", () => {
			...
			const useRealStripe =
				process.env.STRIPE_SECRET_KEY_TEST?.startsWith("sk_test_");

			it.skipIf(!useRealStripe)(
				"should create payment intent with real Stripe API",
				async () => {
