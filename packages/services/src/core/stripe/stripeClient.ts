    ...
    for (let attempt = 0; attempt < RETRY_CONFIG.maxAttempts; attempt++) {
      try {
        ...
        if (!isRetryable) {
          ...
          logger.error(`Operation ${operationName} failed (non-retryable)`, error, {
            operation: operationName,
            attempt: attempt + 1,
        ...
        // Exponential backoff
        const backoffMs =
          RETRY_CONFIG.backoff[attempt] || RETRY_CONFIG.backoff[RETRY_CONFIG.backoff.length - 1]
  ...
  return {
    ...
    products: {
      ...
      list: (
        ...
        callWithResilience(() => getStripeInstance().products.list(...args), 'products.list'),
    },
    prices: {
