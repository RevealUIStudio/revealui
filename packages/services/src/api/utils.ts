...
export const manageSubscriptionStatusChange = async (
	...
	const subscription =
		await protectedStripe.subscriptions.retrieve(subscriptionId);
...
export const handleCustomerCreated = (
	event: StripeWebhookEvent<"customer.created">,
): void => {
...
export const handleCustomerUpdated = (
	event: StripeWebhookEvent<"customer.updated">,
): void => {
	const customer = event.data.object;
	if (!customer.email || typeof customer.email !== "string") {
		throw new Error("Customer missing email");
...
> => {
	...
	try {
		...
		const paymentIntent =
			await protectedStripe.paymentIntents.create(paymentIntentParams);
...
export const createStripeCustomer = async ({
	...
	if (
		...
		try {
			...
			if (revealuiInstance?.logger) {
				revealuiInstance.logger.error(
					`Error creating Stripe customer: ${errorMessage}`,
				);
			} else {
