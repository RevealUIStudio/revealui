...
function getStripe(): Stripe {
	...
	try {
		...
		secretKey = (configModule as unknown as { stripe?: { secretKey?: string } })
			.stripe?.secretKey;
	} catch {
 ...
 *
 * TODO: For multi-instance deployments, consider:
 * - Redis-backed circuit breaker state
 * - Database-backed state using existing DB connection
 * - Distributed state synchronization
 *
 * Current limitation: Circuit breaker state is per-instance only.
 * Multiple instances won't share circuit breaker state.
 */
...
function isCircuitOpen(operationName: string): boolean {
	...
	// If half-open, allow operation (to test if service recovered)
	return false;
}
...
function createProtectedStripe(stripeInstance?: Stripe) {
	...
	async function callWithResilience<T>(
		...
		if (isCircuitOpen(operationName)) {
			logger.error(`Circuit breaker is OPEN for ${operationName}`, undefined, {
				operation: operationName,
				state: "open",
			});
			throw new Error(
				`Circuit breaker is OPEN for ${operationName}. Service may be unavailable. Please try again later.`,
			);
		}
		...
		for (let attempt = 0; attempt < RETRY_CONFIG.maxAttempts; attempt++) {
			try {
				...
				// Log retryable error
				logger.warn(
					`Operation ${operationName} failed (retryable), retrying...`,
					{
						operation: operationName,
						attempt: attempt + 1,
						maxAttempts: RETRY_CONFIG.maxAttempts,
	...
	return {
		billingPortal: {
			sessions: {
				create: (
					...
					callWithResilience(
						() =>
							getStripeInstance().billingPortal.sessions.create(
								params,
								options,
							),
		...
		customers: {
			...
			update: (
				...args: Parameters<Stripe["customers"]["update"]>
			): Promise<Stripe.Customer> =>
				callWithResilience(
					() => getStripeInstance().customers.update(...args),
					"customers.update",
				),
			del: (
				...args: Parameters<Stripe["customers"]["del"]>
			): Promise<Stripe.DeletedCustomer> =>
				callWithResilience(
					() => getStripeInstance().customers.del(...args),
					"customers.del",
				),
			list: (
		...
		products: {
			create: (
				...args: Parameters<Stripe["products"]["create"]>
			): Promise<Stripe.Product> =>
				callWithResilience(
					() => getStripeInstance().products.create(...args),
					"products.create",
				),
			retrieve: (
				...args: Parameters<Stripe["products"]["retrieve"]>
			): Promise<Stripe.Product> =>
				callWithResilience(
			...
			update: (
				...args: Parameters<Stripe["products"]["update"]>
			): Promise<Stripe.Product> =>
				callWithResilience(
					() => getStripeInstance().products.update(...args),
					"products.update",
				),
			list: (
				...args: Parameters<Stripe["products"]["list"]>
			): Promise<Stripe.ApiList<Stripe.Product>> =>
				callWithResilience(
					() => getStripeInstance().products.list(...args),
					"products.list",
				),
		},
		prices: {
			create: (
				...args: Parameters<Stripe["prices"]["create"]>
			): Promise<Stripe.Price> =>
				callWithResilience(
					() => getStripeInstance().prices.create(...args),
					"prices.create",
				),
			retrieve: (
				...
				callWithResilience(
					() => getStripeInstance().prices.retrieve(...args),
					"prices.retrieve",
				),
			update: (
				...args: Parameters<Stripe["prices"]["update"]>
			): Promise<Stripe.Price> =>
				callWithResilience(
					() => getStripeInstance().prices.update(...args),
					"prices.update",
				),
			list: (
				...
				callWithResilience(
					() => getStripeInstance().prices.list(params, options),
					"prices.list",
				),
		},
		subscriptions: {
...
export const __circuitBreakers = circuitBreakers;
export const __CIRCUIT_BREAKER_CONFIG = CIRCUIT_BREAKER_CONFIG;
export const __RETRY_CONFIG = RETRY_CONFIG;
export const __getCircuitBreaker = getCircuitBreaker;
export const __recordFailure = recordFailure;
export const __recordSuccess = recordSuccess;
export const __isCircuitOpen = isCircuitOpen;
/**
