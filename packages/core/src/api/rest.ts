 ...
 */
function hasRevealCorsOrigins(
	config: unknown,
): config is { reveal?: { corsOrigins?: string[] } } {
	return (
	...
	try {
		...
		switch (method) {
			...
			case "PATCH": {
				if (!id) {
					return new Response(
						JSON.stringify({ message: "ID required for update operation" }),
						{
							status: 400,
			...
			case "DELETE":
				if (!id) {
					return new Response(
						JSON.stringify({ message: "ID required for delete operation" }),
						{
							status: 400,
							headers: { "Content-Type": "application/json" },
						},
	...
	try {
		...
		if (pathParts[0] === "collections" && pathParts[1]) {
			...
			if (!revealui.collections[collection]) {
				return new Response(
					JSON.stringify({ message: `Collection '${collection}' not found` }),
					{
						status: 404,
						headers: { "Content-Type": "application/json" },
					},
			...
			if (!revealui.globals[global]) {
				return new Response(
					JSON.stringify({ message: `Global '${global}' not found` }),
					{
						status: 404,
						headers: { "Content-Type": "application/json" },
					},
