...
export function handleDatabaseError(
  ...
  if (error instanceof Error) {
    ...
    if (error.message.includes('unique') || error.message.includes('duplicate')) {
      throw new ApplicationError('Resource already exists', 'DUPLICATE_ERROR', 409, {
        operation,
        ...context,
      })
    }
    ...
    if (error.message.includes('foreign key') || error.message.includes('constraint')) {
      throw new ApplicationError('Invalid reference', 'REFERENCE_ERROR', 400, {
        operation,
        ...context,
      })
    }
    ...
    if (error.message.includes('not found') || error.message.includes('does not exist')) {
      throw new ApplicationError('Resource not found', 'NOT_FOUND', 404, { operation, ...context })
    }
  }
  ...
  throw new ApplicationError('Database operation failed', 'DATABASE_ERROR', 500, {
    operation,
    ...context,
  })
}
