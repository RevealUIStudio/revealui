...
export function convertToRevealUIField(field: Field): RevealUIField {
	...
	if (isTextField(field)) {
		...
		if (field.fields) {
			arrayBaseField.fields = field.fields.map((f) =>
				convertToRevealUIField(f),
			);
		}
	...
	if (revealUIField.type === "text") {
		...
		switch (revealUIField.type) {
			case "checkbox": {
				...
				checkboxBaseField.defaultValue = (
					revealUIField as RevealUICheckboxField
				).defaultValue;
				break;
			}
			case "richText": {
				...
				richTextBaseField.editor = (
					revealUIField as RevealUIRichTextField
				).editor;
				break;
			}
		}
			...
			switch (rule.type) {
				...
				case "max":
					...
					if (typeof value === "number" && value > (rule.value as number)) {
						return (
							rule.message ||
							`${field.label || field.name} must be no more than ${rule.value}`
						);
					}
					break;
				case "pattern":
