 ...
 */
export async function traverseFieldsCore(args, mode) {
    ...
    const results = await Promise.allSettled(fieldsToProcess.map(async (field) => {
        const fieldPath = path ? `${path}.${field.name || ''}` : field.name || 'unnamed';
 ...
 */
async function processField(field, path, mode) {
    ...
    if (field.type === 'array' && field.fields) {
        ...
        await Promise.allSettled(field.fields.map(async (nestedField) => {
    ...
    if (field.type === 'blocks' && field.blocks) {
        ...
        if (Array.isArray(field.blocks)) {
            await Promise.allSettled(field.blocks.map(async (blockUnknown) => {
                const block = blockUnknown;
                if (block &&
                    typeof block === 'object' &&
                    'fields' in block &&
                    Array.isArray(block.fields)) {
                    const blockPath = `${path}.${block.slug || 'block'}`;
                    await Promise.allSettled(block.fields.map(async (blockField) => {
    ...
    if (field.type === 'tabs' && field.tabs) {
        ...
        if (Array.isArray(field.tabs)) {
            await Promise.allSettled(field.tabs.map(async (tabUnknown) => {
                const tab = tabUnknown;
                if (tab && 'fields' in tab && Array.isArray(tab.fields)) {
                    const tabPath = `${path}.${tab.name || 'tab'}`;
                    await Promise.allSettled(tab.fields.map(async (tabField) => {
