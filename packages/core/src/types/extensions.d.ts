 ...
 */
export type RevealUIComponent<P = Record<string, unknown>> = ComponentType<P & {
    revealUI?: {
        tenant?: {
            id: string;
            name: string;
...
export type RevealUIFieldType = 'text' | 'textarea' | 'number' | 'email' | 'password' | 'code' | 'date' | 'checkbox' | 'select' | 'radio' | 'relationship' | 'upload' | 'array' | 'blocks' | 'group' | 'row' | 'collapsible' | 'tabs' | 'richText' | 'json' | 'point' | 'ui';
