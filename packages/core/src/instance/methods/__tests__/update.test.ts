...
describe("update method", () => {
	...
	it("should call collection update method", async () => {
		...
		vi.mocked(
			mockInstance.collections["test-collection"].findByID,
		).mockResolvedValue(mockPreviousDoc);
		vi.mocked(
			mockInstance.collections["test-collection"].update,
		).mockResolvedValue(mockUpdatedDoc);
	...
	it("should throw error if collection not found", async () => {
		...
		await expect(
			update(mockInstance, mockEnsureDbConnected, options),
