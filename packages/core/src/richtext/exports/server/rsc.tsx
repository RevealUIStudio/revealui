...
function serializeNode(
	...
	if (node.type === "tablecell") {
		...
		return (
			<Tag
				key={index}
				colSpan={(node as any).colSpan}
				rowSpan={(node as any).rowSpan}
			>
				{serializeChildren(n.children, options)}
			</Tag>
		);
	...
	if (n.children) {
		return (
			<Fragment key={index}>{serializeChildren(n.children, options)}</Fragment>
		);
	}
...
export function serializeLexicalState(
	...
	return serializeChildren(
		data.root.children as SerializedLexicalNode[],
		options,
	);
}
 ...
 */
export function RscEntryLexicalCell({
	data,
	className,
}: RscEntryLexicalCellProps) {
	if (!data) {
		return null;
	}
 ...
 */
export function RscEntryLexicalField({
	data,
	className,
}: RscEntryLexicalFieldProps) {
	if (!data) {
		return null;
	}
