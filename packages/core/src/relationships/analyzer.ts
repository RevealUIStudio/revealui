	...
	function traverseFields(
		...
		for (const field of fields) {
			...
			if (field.type === "relationship" || field.type === "upload") {
				const metadata = createRelationshipMetadata(
					field,
					fieldPath || "",
					tableName,
					isLocalized,
...
export function validateRelationshipMetadata(
	...
	for (const rel of metadata) {
		...
		if (rel.storageType === "polymorphic" && !Array.isArray(rel.relationTo)) {
			errors.push(
				`Polymorphic relationship ${rel.fieldName} must have array relationTo`,
			);
		}

		if (rel.storageType === "direct_fk" && rel.hasMany) {
			errors.push(
				`Direct FK relationship ${rel.fieldName} cannot have hasMany=true`,
			);
		}

		if (rel.storageType !== "direct_fk" && rel.fkColumnName) {
			errors.push(
				`Non-direct-FK relationship ${rel.fieldName} should not have fkColumnName`,
			);
		}
		...
		if (rel.storageType === "direct_fk" && rel.tableName?.includes("_rels")) {
			errors.push(
				`Direct FK relationship ${rel.fieldName} should not use _rels table`,
			);
		}

		if (
			rel.storageType !== "direct_fk" &&
			rel.tableName &&
			!rel.tableName.includes("_rels")
		) {
			errors.push(
				`Junction table relationship ${rel.fieldName} should use _rels table`,
			);
		}
	}
