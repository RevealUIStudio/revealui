  ...
  describe('collection operations', () => {
    describe('GET (list)', () => {
      ...
      it('calls revealui.find with collection name', async () => {
        ...
        expect(revealui.find).toHaveBeenCalledWith(expect.objectContaining({ collection: 'posts' }))
      })
  ...
  describe('query parameter parsing', () => {
    ...
    describe('sorting', () => {
      it('parses sort parameter', async () => {
        const request = createRequest('http://localhost:3000/api/collections/posts?sort=-createdAt')
        await handleRESTRequest(request, config, revealui)
    ...
    describe('field selection', () => {
      ...
      it('ignores invalid select JSON', async () => {
        const request = createRequest('http://localhost:3000/api/collections/posts?select=not-json')
        await handleRESTRequest(request, config, revealui)
