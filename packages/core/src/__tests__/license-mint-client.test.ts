...
describe('withPerpetualSiteCaps (GAP-448)', () => {
  ...
  it('bakes Agency perpetual maxSites 10 and maxUsers 100 when omitted', () => {
    expect(
      withPerpetualSiteCaps({
        tier: 'max',
        customerId: 'cus_1',
    ...
    ).toMatchObject({ maxSites: 10, maxUsers: 100 });
  });

  it('bakes pro perpetual maxSites 5 and maxUsers 25 when omitted', () => {
    expect(
      withPerpetualSiteCaps({
        tier: 'pro',
        customerId: 'cus_1',
    ...
    ).toMatchObject({ maxSites: 5, maxUsers: 25 });
  });
