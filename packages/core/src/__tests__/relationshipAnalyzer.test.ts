...
describe("Relationship Field Analyzer", () => {
	it("should extract all relationship fields with correct metadata", () => {
		...
		for (
			let i = 0;
			i < Math.min(result.length, expectedRelationships.length);
			i++
		) {
			...
			expect(JSON.stringify(actual.relationTo)).toBe(
				JSON.stringify(expected.relationTo),
			);
	...
	it("should detect polymorphic relationships", () => {
		const result = getRelationshipFields(testCollectionConfig);
		const relatedContentRel = result.find(
			(r) => r.fieldName === "relatedContent",
		);
	...
	it("should handle nested relationship fields in arrays", () => {
		const result = getRelationshipFields(testCollectionConfig);
		const tagRel = result.find(
			(r) => r.fieldName === "tag" && r.path === "tags.tag",
		);
