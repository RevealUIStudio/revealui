...
class ZombieDetector {
  ...
  private detectionInterval: number =
    DEFAULT_MONITORING_CONFIG.zombieDetectionInterval;
  ...
  async scan(): Promise<ZombieProcess[]> {
    ...
    try {
      ...
      const { stdout } = await execAsync(
        "ps axo pid,ppid,comm,state | grep -E ' (Z|z)' || true"
      );
  ...
  private async attemptCleanup(zombie: ZombieProcess): Promise<boolean> {
    try {
      ...
      const { stdout: parentExists } = await execAsync(
        `ps -p ${zombie.ppid} -o pid= || true`
      );
