...
export function createTableContractRegistry<T extends Database>(map: TableContractMap<T>) {
  return {
    ...
    validate<K extends keyof T['public']['Tables']>(
      ...
      return contract.validate(row as RowType) as T['public']['Tables'][K] extends {
        Row: infer R
      }
