...
describe('Contract Error Handling', () => {
  describe('ConfigValidationError', () => {
    it('formats error message with config type', () => {
      const mockZodError = {
        issues: [
          { path: ['slug'], message: 'Required', code: 'invalid_type' as const },
        ],
      };
    ...
    it('provides helper methods for issue access', () => {
      const mockZodError = {
        issues: [
          { path: ['fields', 0, 'name'], message: 'Required', code: 'invalid_type' as const },
          { path: ['slug'], message: 'Invalid format', code: 'custom' as const },
        ],
      };
...
describe('Plugin Field Extensions', () => {
  ...
  it('applies collection-specific fields', () => {
    registerPluginExtension({
      ...
      collectionFields: {
        'test-collection': [
          { type: 'number', name: 'viewCount' } as any,
        ],
      },
    });
