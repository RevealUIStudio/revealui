...
describe("Config Contract", () => {
	describe("Validation", () => {
		it("validates valid config with minimal required fields", () => {
			...
			if (!result.success) {
				expect(
					result.errors.issues.some((e) => e.path.includes("collections")),
				).toBe(true);
			}
		});
		...
		it("rejects invalid serverURL format", () => {
			...
			if (!result.success) {
				...
				expect(
					result.errors.issues.some((e) => e.path.includes("serverURL")),
				).toBe(true);
			}
		});
		...
		it("rejects config without secret", () => {
			...
			if (!result.success) {
				expect(
					result.errors.issues.some((e) => e.path.includes("secret")),
				).toBe(true);
			}
		});
		...
		it("rejects invalid globals type (not array)", () => {
			...
			if (!result.success) {
				expect(
					result.errors.issues.some((e) => e.path.includes("globals")),
				).toBe(true);
			}
		});
		...
		it("rejects invalid email address", () => {
			...
			if (!result.success) {
				expect(
					result.errors.issues.some((e) => e.path.includes("fromAddress")),
				).toBe(true);
			}
		});
		...
		it("rejects wrong types for optional fields", () => {
			...
			expect(result.success).toBe(false);
			if (!result.success) {
				expect(result.errors.issues.some((e) => e.path.includes("debug"))).toBe(
					true,
				);
			}
		});
