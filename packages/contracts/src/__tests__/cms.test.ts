	...
	describe("Field Schemas", () => {
		...
		describe("FieldAdminConfigSchema", () => {
			...
			it("should validate layout", () => {
				expect(
					FieldAdminConfigSchema.safeParse({ layout: "horizontal" }).success,
				).toBe(true);
				expect(
					FieldAdminConfigSchema.safeParse({ layout: "vertical" }).success,
				).toBe(true);
				expect(
					FieldAdminConfigSchema.safeParse({ layout: "invalid" }).success,
				).toBe(false);
			});

			it("should validate width as string or number", () => {
				expect(FieldAdminConfigSchema.safeParse({ width: 100 }).success).toBe(
					true,
				);
				expect(FieldAdminConfigSchema.safeParse({ width: "50%" }).success).toBe(
					true,
				);
			});
		...
		describe("Type Guards", () => {
			...
			it("isRelationshipField should identify relationship fields", () => {
				expect(isRelationshipField({ name: "x", type: "relationship" })).toBe(
					true,
				);
			...
			it("isArrayField should identify array fields", () => {
				expect(isArrayField({ name: "x", type: "array", fields: [] })).toBe(
					true,
				);
			...
			it("isGroupField should identify group fields", () => {
				expect(isGroupField({ name: "x", type: "group", fields: [] })).toBe(
					true,
				);
			...
			it("hasNestedFields should identify fields with nested fields", () => {
				expect(hasNestedFields({ name: "x", type: "array", fields: [] })).toBe(
					true,
				);
				expect(hasNestedFields({ name: "x", type: "group", fields: [] })).toBe(
					true,
				);
				expect(hasNestedFields({ type: "row", fields: [] })).toBe(true);
				expect(hasNestedFields({ type: "collapsible", fields: [] })).toBe(true);
				expect(hasNestedFields({ type: "tabs", tabs: [{ fields: [] }] })).toBe(
					true,
				);
	...
	describe("Collection Schemas", () => {
		...
		describe("CollectionAdminConfigSchema", () => {
			...
			it("should validate useAsTitle", () => {
				...
				expect(CollectionAdminConfigSchema.safeParse(config).success).toBe(
					true,
				);
			});
		...
		describe("AuthConfigSchema", () => {
			...
			it("should validate cookie sameSite values", () => {
				expect(
					AuthConfigSchema.safeParse({ cookies: { sameSite: "strict" } })
						.success,
				).toBe(true);
				expect(
				...
				expect(
					AuthConfigSchema.safeParse({ cookies: { sameSite: "invalid" } })
						.success,
				).toBe(false);
			});
		});
		...
		describe("VersionsConfigSchema", () => {
			...
			it("should validate drafts", () => {
				expect(VersionsConfigSchema.safeParse({ drafts: true }).success).toBe(
					true,
				);
				expect(VersionsConfigSchema.safeParse({ drafts: false }).success).toBe(
					true,
				);
				expect(
		...
		describe("Collection Factory Functions", () => {
			...
			it("createAuthCollectionConfig should accept auth options", () => {
				...
				if (typeof config.auth === "object" && config.auth !== null) {
					expect((config.auth as Record<string, unknown>).tokenExpiration).toBe(
						7200,
					);
			...
			it("createUploadCollectionConfig should accept upload options", () => {
				...
				if (typeof config.upload === "object" && config.upload !== null) {
					expect((config.upload as Record<string, unknown>).staticDir).toBe(
						"./uploads/images",
					);
	...
	describe("Global Schemas", () => {
		...
		describe("GlobalAdminConfigSchema", () => {
			...
			it("should validate hidden", () => {
				expect(
					GlobalAdminConfigSchema.safeParse({ hidden: true }).success,
				).toBe(true);
				expect(
					GlobalAdminConfigSchema.safeParse({ hidden: false }).success,
				).toBe(true);
			});
		});
		...
		describe("GlobalVersionsConfigSchema", () => {
			...
			it("should validate maxPerDoc", () => {
				expect(GlobalVersionsConfigSchema.safeParse({ max: 5 }).success).toBe(
					true,
				);
			});
			...
			it("should validate drafts", () => {
				expect(
					GlobalVersionsConfigSchema.safeParse({ drafts: true }).success,
				).toBe(true);
			});
		});
		...
		describe("SanitizedGlobalConfigSchema", () => {
			it("should validate with flattened fields", () => {
				const config = {
					...
					fields: [{ name: "siteName", type: "text" }],
					flattenedFields: [
						{ name: "siteName", type: "text", path: "siteName" },
					],
				};
		...
		describe("Global Factory Functions", () => {
			...
			it("createGlobalConfig should handle nested fields", () => {
				...
				expect((config.fields[0] as { fields: Field[] }).fields).toHaveLength(
					2,
				);
			});
		});
