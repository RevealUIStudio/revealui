					...
					errors: [
						{
							...
							message:
								message || `Field '${field}' cannot be changed after creation`,
							...(field && { field }),
							constraint,
						},
					],
						...
						errors: [
							{
								...
								message:
									message || `Value must match pattern ${params.pattern}`,
								...(field && { field }),
								constraint,
							},
						],
...
export function validateAction(
	...
	const actionDef = entity.agent.actions?.find(
		(a: AgentActionDefinition) => a.name === action,
	);
	if (!actionDef) {
		return {
			success: false,
	...
	if (actionDef.requiredCapabilities) {
		const capabilityResult = checkCapabilities(
			agent,
			actionDef.requiredCapabilities,
		);
