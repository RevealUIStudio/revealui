...
const pct = components.length ? Math.round((claimed.length / components.length) * 100) : 0;
...
lines.push(`**Assessed** ${claimed.length} of ${components.length} components (${pct}%)`);
lines.push('');
...
if (gaps.length > 0) {
  ...
  lines.push('');
  lines.push(`${gaps.length} of ${components.length} components carry no conformance claim. They are`);
...
console.log(`  ${claimed.length}/${components.length} components assessed (${pct}%) · ${criteria.size} distinct criteria`);
