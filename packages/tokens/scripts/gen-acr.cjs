...
/**
 * Files that are not public showcase surfaces.
 * dropdown-trigger is an internal Dropdown primitive (see file header).
 */
const INTERNAL_COMPONENT_FILES = new Set(['dropdown-trigger.tsx']);

/**
 * Dual-layer / alias files covered by an existing showcase slug.
 * Keeps the ACR "documented" count honest without inventing phantom pages.
 */
const FILE_SHOWCASE_SLUG = {
  'checkbox-headless.tsx': 'checkbox',
  'Checkbox.tsx': 'checkbox',
  'input-headless.tsx': 'input',
  'Input.tsx': 'input',
  'select-headless.tsx': 'select',
  'Select.tsx': 'select',
  'textarea-headless.tsx': 'textarea',
  'Textarea.tsx': 'textarea',
  'BuiltWithRevealUI.tsx': 'built-with',
  'Button.tsx': 'button',
  'Card.tsx': 'card',
  'FormLabel.tsx': 'form-label',
  'Label.tsx': 'label',
  'LinkButton.tsx': 'linkbutton',
  'icon.tsx': 'icons',
  'brand-mark.tsx': 'brand-mark',
  'wordmark.tsx': 'wordmark',
};

function listShippedComponentFiles() {
  const dir = join(ROOT, 'packages', 'presentation', 'src', 'components');
  if (!existsSync(dir)) return [];
  return readdirSync(dir).filter(
    (f) => f.endsWith('.tsx') && !f.startsWith('_') && !INTERNAL_COMPONENT_FILES.has(f),
  );
}

function countComponents() {
  const files = listShippedComponentFiles();
  return files.length || null;
}

/** Files with no registry component showcase covering them. */
function uncoveredComponentFiles(componentSlugs) {
  const slugSet = new Set(componentSlugs);
  const uncovered = [];
  for (const file of listShippedComponentFiles()) {
    const mapped = FILE_SHOWCASE_SLUG[file];
    if (mapped) {
      if (!slugSet.has(mapped)) uncovered.push(file);
      continue;
    }
    // default: kebab of stem
    const stem = file.replace(/\.tsx$/, '');
    const kebab = stem
      .replace(/([a-z0-9])([A-Z])/g, '$1-$2')
      .replace(/_/g, '-')
      .toLowerCase();
    if (!slugSet.has(kebab) && !slugSet.has(stem.toLowerCase())) uncovered.push(file);
  }
  return uncovered;
}
...
const uncoveredFiles = uncoveredComponentFiles(components.map((c) => c.slug));
const undocumented = uncoveredFiles.length;
...
lines.push(
  `**Documented** ${denom - undocumented} of ${denom} shipped components have showcase coverage (${components.length} registry pages)`,
);
lines.push('');
...
if (undocumented > 0) {
  ...
  lines.push('');
  for (const f of uncoveredFiles) lines.push(`- \`${f}\``);
  lines.push('');
  // keep following prose if any was truncated — rewritten below intentionally
  lines.push('conformance on the rest.');
  lines.push('');
}
