...
- \`.revealui/adapters/grok/hooks/cap-tool-output.json\`
- \`.revealui/adapters/grok/token-budget.json\`

\`token-budget.json\` is the compact contract from \`src/token-budget.ts\`
(parent compact tokens, snapshot headroom, grep/shell output cap). RevKit
copies the hook allowlist, installs \`cap-tool-output.js\`, and writes
\`compaction_at_tokens\` plus \`auto_compact_threshold_percent\` from that
file. Do not hand-edit the integers in \`~/.grok/config.toml\`.
...
| SessionStart | \`revealui-harnesses session adapter grok\` (control layer first, then this adapter) |
...
`,
  ...
  const budgetRel = join(MANAGER_DIR, 'adapters', 'grok', 'token-budget.json');
  const budgetAbs = join(projectRoot, budgetRel);
  mkdirSync(dirname(budgetAbs), { recursive: true });
  writeFileSync(budgetAbs, tokenBudgetJsonText(), 'utf-8');
