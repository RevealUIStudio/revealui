...
import { grokRulePathForDefinitionId } from '../content/generators/grok.js';
 ...
 * rules into `.grok/rules/`. Machine install: copy
...
\`\`\`bash
mkdir -p ~/.grok/hooks
cp .revealui/adapters/grok/hooks/pre-tool.json ~/.grok/hooks/
...
`,
  ...
  // Deny path only. Do not clobber operator SessionStart extras.
  const homePreTool = join(homedir(), '.grok', 'hooks', 'pre-tool.json');
  try {
    mkdirSync(dirname(homePreTool), { recursive: true });
    const body = GROK_HOOK_FILES['pre-tool.json'];
    writeFileSync(homePreTool, body.endsWith('\n') ? body : `${body}\n`, 'utf-8');
  } catch {
    // Machine home may be read-only in CI.
  }
