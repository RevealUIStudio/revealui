...
describe('harness-config-paths', () => {
  ...
  it('returns SSD config path with custom base', () => {
    const p = getSsdConfigPath('claude-code', '/mnt/e/.revealui')
    expect(p).toBeDefined()
    expect(p).toContain('harness-configs/claude-code')
    expect(p).toContain('settings.json')
  })

  it('returns undefined SSD path for unknown harness', () => {
    expect(getSsdConfigPath('unknown', '/mnt/e/.revealui')).toBeUndefined()
  })
})
