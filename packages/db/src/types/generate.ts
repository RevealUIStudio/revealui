	...
	if (discoveryErrors.length > 0) {
		for (const error of discoveryErrors) {
			const location = error.position
				...
				: error.file;
			console.warn(
				`⚠️  ${location}: ${error.message}${error.context ? ` (${error.context})` : ""}`,
			);
		}
	}
...
export const ${relationshipsVar}: readonly Relationship[] = []`;
			...
			const relationshipsStr = relationships
				.map((rel) => {
					...
					const refColumnsStr = rel.referencedColumns
						.map((c) => `'${c}'`)
						.join(", ");
...
${relationshipsStr},
	...
	if (relationshipsIndex > -1) {
		...
		const after = header.substring(
			relationshipsIndex + relationshipsMarker.length,
		);
...
try {
	...
	if (VERBOSE_LOGGING) {
		console.log(
			`✅ Generated Database type: ${outputPath.replace(rootDir, ".")}`,
		);
