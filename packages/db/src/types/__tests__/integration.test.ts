...
describe("Database Type Integration Tests", () => {
	...
	beforeAll(() => {
		...
		try {
			...
			hasDatabase = false;
			console.warn(
				"⚠️  Database not available - skipping integration tests that require database",
			);
		}
	});
	...
	it.skipIf(!hasDatabase || !db)(
		"should query users table with correct types",
		async () => {
			...
			if (users.length > 0) {
				...
				type UsersRelationships =
					Database["public"]["Tables"]["users"]["Relationships"];
	...
	it.skipIf(!hasDatabase || !db)(
		...
		async () => {
			...
			if (sessions.length > 0) {
				...
				type SessionsRelationships =
					Database["public"]["Tables"]["sessions"]["Relationships"];
