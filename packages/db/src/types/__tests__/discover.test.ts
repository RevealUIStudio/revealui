...
`,
	...
	describe("discoverTables", () => {
		...
		it("should discover sessions table", () => {
			...
			const sessionsTable = result.tables.find(
				(t) => t.variableName === "sessions",
			);
	...
	describe("validateTables", () => {
		...
		it("should detect duplicate variable names", () => {
			...
			expect(
				validation.errors.some((e) => e.includes("Duplicate variable name")),
			).toBe(true);
		});
		...
		it("should detect duplicate table names", () => {
			...
			expect(validation.valid).toBe(false);
			expect(
				validation.errors.some((e) => e.includes("Duplicate table name")),
			).toBe(true);
		});
		...
		it("should validate table name format (snake_case)", () => {
			...
			expect(validation.valid).toBe(false);
			expect(
				validation.errors.some((e) => e.includes("Invalid table name format")),
			).toBe(true);
		});
