...
describe("@revealui/config", () => {
	...
	describe("Validation", () => {
		...
		it("should validate URL format", () => {
			...
			expect(result.success).toBe(false);
			expect(
				result.errors.some((e) => e.variable === "REVEALUI_PUBLIC_SERVER_URL"),
			).toBe(true);
		});
		...
		it("should validate PostgreSQL URL format", () => {
			...
			expect(
				result.errors.some(
					(e) =>
						e.variable === "POSTGRES_URL" && e.message.includes("PostgreSQL"),
				),
			).toBe(true);
		});
		...
		it("should accept DATABASE_URL as fallback for POSTGRES_URL", () => {
			...
			try {
				...
				expect(loaded.POSTGRES_URL).toBe(
	...
	describe("Error Messages", () => {
		...
		it("should include warnings for deprecated variables", () => {
			...
			expect(
				result.warnings.some(
					(w) =>
						w.includes("REVEALUI_WHITELISTORIGINS") && w.includes("deprecated"),
				),
			).toBe(true);
		});
		...
		it("should include warnings for DATABASE_URL usage", () => {
			...
			expect(
				result.warnings.some(
					(w) => w.includes("DATABASE_URL") && w.includes("POSTGRES_URL"),
				),
			).toBe(true);
		});
	});
	...
	describe("Config Structure", () => {
		...
		it("should provide stripe config", () => {
			...
			expect(config.stripe.publishableKey).toBe(
				validEnv.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY,
			);
		});
		...
		it("should provide reveal config", () => {
			...
			expect(config.reveal.publicServerURL).toBe(
				validEnv.REVEALUI_PUBLIC_SERVER_URL,
			);
		});
	...
	describe("Optional Variables", () => {
		...
		it("should handle optional CORS origins", () => {
			...
			expect(config.reveal.corsOrigins).toEqual([
				"http://localhost:3000",
				"http://localhost:3001",
			]);
		});
