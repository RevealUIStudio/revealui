/**
 * Browser cookie-consent store.
 *
 * Client-safe (no `node:`). The previous `CookieConsentManager` lived in
 * `gdpr.ts` behind `@revealui/security/server`, so the marketing site and
 * admin could not import it. This module is the browser consent SSOT:
 * first-party cookie + localStorage, opt-in optional categories, GPC/DNT.
 */
